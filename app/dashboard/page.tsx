...
const EventManagement = dynamic(
  () =>
    import('@/components/calendar/EventManagement').then((mod) => ({
      default: mod.EventManagement,
    })),
  { loading: () => <div className="h-full w-full bg-muted animate-pulse" /> }
);
...
const ExternalEventsPanel = dynamic(
  () =>
    import('@/components/calendar/ExternalEventsPanel').then((mod) => ({
      default: mod.ExternalEventsPanel,
    })),
  { loading: () => <div className="w-80 border-l border-border bg-card p-4 animate-pulse" /> }
);
import type { CalendarView } from '@/components/dashboard/ViewSwitcher';
...
const EnhancedAIAssistant = dynamic(
  () =>
    import('@/components/ai/EnhancedAIAssistant').then((mod) => ({
      default: mod.EnhancedAIAssistant,
    })),
...
export default function DashboardPage() {
  ...
  const calendarEvents = useMemo(() => {
    ...
    if (filters.tags.size > 0) {
      filtered = filtered.filter(
        (event) => event.tags?.some((tag) => filters.tags.has(tag)) || false
      );
    }
  ...
  const handleEventDelete = useCallback(
    async (id: string) => {
      const numericId = Number.parseInt(id) || events?.find((e) => e.convexId === id)?.id;
      if (numericId) {
  ...
  const _isYearView = currentView === 'year';
    ...
    >
                    ...
                    onEventUpdate={async (event) => {
                      if (updateEvent) {
                        const existingEvent = events?.find(
                          (e) => e.convexId === event.id || String(e.id) === event.id
                        );
                        if (existingEvent) {
      ...
      {process.env.NODE_ENV === 'development' && (
        <Suspense fallback={null}>
          <PerformanceMonitor eventCount={calendarEvents.length} position="bottom-left" />
        </Suspense>
      )}
