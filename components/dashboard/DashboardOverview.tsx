...
import { format, isToday, isSameDay, startOfDay, endOfDay, differenceInDays, startOfWeek, endOfWeek } from "date-fns"
...
export function DashboardOverview({ showBalance = true, year = new Date().getFullYear(), className }: DashboardOverviewProps) {
  const { user } = useUser()
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case 'call': return <Bell className="h-4 w-4" />
  ...
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'added': return <Plus className="h-4 w-4 text-blue-500" />
      case 'updated': return <Settings className="h-4 w-4 text-yellow-500" />
  ...
  const metrics = useMemo(() => {
    ...
    const weekEvents = getEventsInRange(startOfThisWeek, endOfThisWeek)
    ...
    const productivity = plannedToday > 0 ? Math.round((completedTodayCount / plannedToday) * 100) : 0
  ...
  const insights = useMemo(() => {
    ...
    const avgDuration = weekEvents.length > 0 
      ? weekEvents.reduce((sum, event) => sum + (event.endDate.getTime() - event.startDate.getTime()), 0) / weekEvents.length / (1000 * 60)
      : 0
    ...
    <div className="w-full max-w-7xl mx-auto p-6 space-y-6 bg-background">
      ...
      <div ref={metricsGridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {hideMetrics ? "•••" : metrics.totalEvents}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {hideMetrics ? "•••" : metrics.upcomingEvents}
            </div>
            <p className="text-xs text-muted-foreground">
              {conflicts.length} high priority
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {hideMetrics ? "•••" : metrics.completedToday}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-4">
            ...
            {upcomingEventsDisplay.length > 0 ? upcomingEventsDisplay.map((event) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div key={index} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className={`text-xs ${insight.trend === 'up' ? 'text-green-500' : 'text-red-500'}`}>
                      {insight.change}
                    </span>
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {calendarSyncStatus.length > 0 ? calendarSyncStatus.map((calendar, index) => (
              <div key={index} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${
                    calendar.status === 'syncing' ? 'bg-yellow-500 animate-pulse' : 
                    calendar.status === 'synced' ? 'bg-green-500' : 'bg-destructive'
                  }`} />
                  <span className="font-medium">{calendar.name}</span>
                </div>
                <div className="text-right">
                  <div className={`text-xs ${
                    calendar.status === 'syncing' ? 'text-yellow-500' :
                    calendar.status === 'synced' ? 'text-green-500' : 'text-destructive'
                  }`}>
                    {calendar.status === 'syncing' ? 'Syncing...' : 
                     calendar.status === 'synced' ? 'Synced' : 'Error'}
                  </div>
      ...
      <Card>
        ...
        <CardContent>
          <Progress 
            value={hideMetrics ? 0 : metrics.productivity} 
