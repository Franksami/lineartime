...
const SyncQueueMonitor: React.FC = () => {
  ...
  const generateMockJobs = (): SyncJob[] => {
    ...
    const operations: SyncJob['operation'][] = [
      'full_sync',
      'incremental_sync',
      'webhook_update',
      'token_refresh',
    ];
    const statuses: SyncJob['status'][] = [
      'pending',
      'processing',
      'completed',
      'failed',
    ...
    return Array.from({ length: 12 }, (_, i) => {
      ...
      const startedAt =
        status !== 'pending' ? new Date(createdAt.getTime() + Math.random() * 300000) : undefined;
      const completedAt =
        status === 'completed'
          ? new Date((startedAt || createdAt).getTime() + Math.random() * 600000)
          : undefined;
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            Real-time synchronization job monitoring and management
          </p>
        </div>
          ...
          <ScrollArea className="h-96">
            <div className="space-y-3">
              {jobs
                ...
                .map((job) => (
                  <div key={job.id} className="p-4 bg-muted/50 rounded-lg border border-border/50">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{job.createdAt.toLocaleTimeString()}</p>
                      </div>
                      {job.startedAt && (
                        <div>
                          <p className="text-muted-foreground">Started</p>
                          <p className="font-medium">{job.startedAt.toLocaleTimeString()}</p>
                        </div>
                      )}
                      {job.completedAt && (
                        <div>
                          <p className="text-muted-foreground">Completed</p>
                          <p className="font-medium">{job.completedAt.toLocaleTimeString()}</p>
                        </div>
                      )}
                      <div>
