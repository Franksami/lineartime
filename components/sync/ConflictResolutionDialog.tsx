...
export function ConflictResolutionDialog({
    ...
    switch (strategy) {
      case 'local':
        resolvedEvent = {
          ...
          version: Math.max(currentConflict.localEvent.version, currentConflict.remoteEvent.version) + 1,
  ...
  const handleResolveAll = async () => {
    ...
    try {
      ...
      for (const resolution of resolutionArray) {
        const eventId = conflicts.find(c => c.suggested === resolution || 
          resolutions.get(c.eventId) === resolution)?.eventId
  ...
  const renderEventCard = (event: SyncableEvent, title: string, variant: 'local' | 'remote' | 'merged') => {
    const iconColor = {
      local: 'text-blue-500',
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Badge variant="outline">
                Event: {currentConflict.localEvent.title}
              </Badge>
              <Badge variant={currentConflict.conflictType === 'content' ? 'destructive' : 'secondary'}>
          ...
          <Tabs value={selectedStrategy} onValueChange={handleStrategyChange}>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <TabsContent value="merge" className="mt-0 col-span-full md:col-span-1">
                {currentResolution && renderEventCard(currentResolution.resolvedEvent, 'Merged Version', 'merged')}
              </TabsContent>
            </div>
          </Tabs>
