...
export function AssistantPanel({
  ...
  const [_parsedEvent, setParsedEvent] = React.useState<Partial<Event> | null>(null);
  const [_aiSuggestions, _setAiSuggestions] = React.useState<string[]>([]);
              ...
              >
                ...
                {(chats || []).map((c: any) => (
                  <option key={c._id} value={c._id}>
                    {c.title || 'Chat'} · {c.messageCount} msgs ·{' '}
                        ...
                        <Message from={message.role} key={message.id}>
                          <MessageContent>
                            {(message.parts || []).map((part: any, i: number) => {
                              ...
                              if (part.type === 'code')
                                return (
                                  <CodeBlock key={i} language={part.language || 'tsx'}>
                                    {part.text}
                                  </CodeBlock>
                                );
                              if (part.type === 'citation')
                                return (
                                  <InlineCitation key={i} index={part.index} href={part.url} />
                                );
                              ...
                              if (part.type === 'image')
                                return <Image key={i} src={part.url} alt={part.alt || 'Image'} />;
                              if (part.type === 'link-preview')
                                return <WebPreview key={i} defaultUrl={part.url} />;
                                          ...
                                          isSuggest && output?.suggestions?.length ? (
                                            <div className="p-2">
                                              {output.suggestions.map((s: any, idx: number) => (
                                                <div
                                                  key={idx}
                                                  className="flex items-center justify-between gap-3 border-b last:border-0 p-2"
                                                >
                            ...
                            {message.role === 'assistant' && (
                              <Actions className="mt-2">
                                ...
                                <Action
                                  tooltip="Dislike"
                                  onClick={() => {
                                    logEvent({ eventType: 'dislike' }).catch(() => {});
                                  }}
                                >
                          ...
                          onClick={() => {
                            if (suggestion.startsWith('AI:')) {
                              ...
                              if (suggestion.includes('Optimize')) {
                                ...
                                sendMessage({
                                  text: "I'll analyze your calendar for conflicts and suggest resolutions.",
                                });
                              }
                            } else {
