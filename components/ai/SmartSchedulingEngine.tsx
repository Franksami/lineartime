...
import { format, addMinutes, isAfter, isBefore, startOfDay, endOfDay, isSameDay, differenceInMinutes, addDays } from 'date-fns'
import { z } from 'zod'
      ...
      while (isBefore(addMinutes(currentTime, duration), dayEnd)) {
        ...
        if (conflicts.length === 0) {
          ...
          for (const event of eventsOnDay) {
            if (Math.abs(differenceInMinutes(currentTime, event.endDate)) < SCHEDULING_PREFERENCES.bufferTime ||
                Math.abs(differenceInMinutes(slotEnd, event.startDate)) < SCHEDULING_PREFERENCES.bufferTime) {
              hasBuffer = false
              confidence -= 20
              break
            }
          ...
          const reason = [
            confidence >= 80 ? '✨ Optimal time slot' :
            confidence >= 60 ? '👍 Good time slot' :
            confidence >= 40 ? '⚠️ Acceptable slot' : '❌ Poor timing',
  ...
  const calendarTools = {
    ...
    analyzeSchedule: tool({
      ...
      parameters: z.object({
        ...
        focusArea: z.enum(['efficiency', 'balance', 'productivity', 'meetings']).describe('Area to focus the analysis on')
      }),
      execute: async ({ dateRange, focusArea }) => {
        ...
        const relevantEvents = events.filter(event => 
          isAfter(event.startDate, startDate) && isBefore(event.endDate, endDate)
        )
        ...
        const totalFocusTime = focusTimeEvents.reduce((acc, e) => acc + differenceInMinutes(e.endDate, e.startDate), 0)
        
        const analysis = {
          totalEvents: relevantEvents.length,
          totalMeetingTime: Math.round(totalMeetingTime / 60 * 10) / 10,
          totalFocusTime: Math.round(totalFocusTime / 60 * 10) / 10,
          averageEventLength: relevantEvents.length > 0 ? 
            Math.round(relevantEvents.reduce((acc, e) => acc + differenceInMinutes(e.endDate, e.startDate), 0) / relevantEvents.length) : 0,
          focusArea,
          recommendations: []
        }
        ...
        if (focusArea === 'balance' && totalMeetingTime > totalFocusTime * 2) {
          analysis.recommendations.push('Consider blocking more focus time to balance meeting load')
        }
        if (focusArea === 'efficiency' && analysis.averageEventLength < 30) {
          analysis.recommendations.push('Many short events detected - consider batching similar tasks')
        }
        if (focusArea === 'productivity' && focusTimeEvents.length === 0) {
          analysis.recommendations.push('No dedicated focus time found - schedule deep work blocks')
        }
    ...
    scheduleEvent: tool({
      ...
      parameters: z.object({
        ...
        duration: z.number().describe('Duration in minutes'),
        eventType: z.enum(['meeting', 'focus', 'break', 'task', 'personal']).describe('Type of event'),
      ...
      execute: async ({ title, duration, eventType, priority, description, preferredTime, preferredDate }) => {
        const prefDate = preferredDate ? new Date(preferredDate) : undefined
        ...
        const eventCategory = eventType === 'meeting' ? 'work' : 
                             eventType === 'focus' ? 'effort' :
                             eventType === 'personal' ? 'personal' : 'note'
...
Be helpful, concise, and always prioritize the user's productivity and work-life balance.`,
  ...
  const generateSmartSuggestions = useCallback(() => {
    ...
    const lateMeetings = todayEvents.filter(event => 
      event.category === 'work' && event.startDate.getHours() >= 17
    )
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setAiPrompt('Analyze my schedule for this week and suggest improvements')}
            >
              <Brain className="h-3 w-3 mr-1" />
              Analyze Schedule
            </Button>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            {(() => {
              ...
              const totalTime = todayEvents.reduce((acc, e) => acc + differenceInMinutes(e.endDate, e.startDate), 0)
              ...
              return (
                <>
                  ...
                  <div>
                    <div className="text-2xl font-bold text-green-600">{focusTime.length}</div>
                    <div className="text-xs text-muted-foreground">Focus Blocks</div>
                  </div>
                  <div>
                    <div className="text-2xl font-bold text-orange-600">{Math.round(totalTime / 60 * 10) / 10}h</div>
