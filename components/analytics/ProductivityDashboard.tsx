...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, getDay, getDayOfYear } from 'date-fns'
...
export function ProductivityDashboard({ 
  ...
  const analyticsData = useMemo(() => {
    ...
    const dayData: DayData[] = allDays.map(date => {
      ...
      const categories = dayEvents.reduce((acc, event) => {
    ...
    const monthlyData = Array.from({ length: 12 }, (_, monthIndex) => {
      ...
      const monthDays = dayData.filter(day => 
      ...
      return {
        ...
        avgProductivity: monthDays.reduce((sum, day) => sum + day.productivity, 0) / monthDays.length,
        categories: monthDays.reduce((acc, day) => {
    ...
    return {
      ...
      mostProductiveDay: dayData.reduce((max, day) => 
        day.productivity > max.productivity ? day : max, dayData[0]
      )
    }
  }, [events, year])
      ...
      g.append('rect')
        ...
        .on('mouseover', function(event) {
          ...
          d3.select('body').append('div')
            ...
            .duration(200)
            .style('opacity', 1)
            .text(`${format(day.date, 'MMM d')}: ${day.count} events (${day.productivity.toFixed(1)} productivity)`)
        })
    ...
    <div className={cn('space-y-6', className)}>
        ...
        <Card>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary">{analyticsData.mostProductiveDay.productivity.toFixed(1)}</Badge>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <svg ref={heatmapRef} className="w-full" style={{ minWidth: '800px' }}></svg>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <svg ref={trendsRef} className="w-full"></svg>
          </CardContent>
        </Card>
