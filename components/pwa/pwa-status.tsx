    ...
    <div className="fixed top-4 right-4 z-30 flex flex-col gap-2 sm:top-16 sm:right-4">
      ...
      {isInstalled && (
        <div className="flex items-center gap-1">
          <Badge variant={notificationsEnabled ? "default" : "outline"} className="flex items-center gap-1">
            {notificationsEnabled ? <Bell className="w-3 h-3" /> : <BellOff className="w-3 h-3" />}
            {notificationsEnabled ? "Notifications On" : "Notifications Off"}
          </Badge>
          {!notificationsEnabled && (
            <Button size="sm" variant="ghost" onClick={handleEnableNotifications} className="h-6 px-2 text-xs">
              Enable
            </Button>
          )}
        </div>
