    ...
    >
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex flex-col items-center gap-2">
          {metric.controversy_level === "high" && <Eye className="w-5 h-5 text-red-500" />}
          {metric.controversy_level === "moderate" && <Target className="w-5 h-5 text-orange-500" />}  
          {metric.controversy_level === "standard" && <Zap className="w-5 h-5 text-blue-500" />}
...
const EliteTaskCard: React.FC<{ task: EliteTask }> = ({ task }) => {
  const priorityConfig = {
    billionaire: { color: "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300", icon: "👑" },
    millionaire: { color: "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300", icon: "💎" },
    entrepreneur: { color: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300", icon: "⚡" }
  };

  const statusConfig = {
    cheating: { color: "text-red-400", icon: <Skull className="w-4 h-4" />, label: "AI Cheating" },
    optimizing: { color: "text-orange-400", icon: <Brain className="w-4 h-4" />, label: "Optimizing" },
    dominated: { color: "text-green-400", icon: <Trophy className="w-4 h-4" />, label: "Dominated" }
  };
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        task.controversy_required ? "border-red-500/30 bg-red-500/5" : "border-border/30 bg-card/30"
      )}
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            {task.controversy_required && <Eye className="w-4 h-4 text-red-500" />}
          </div>
...
const ProductivityCheatCard: React.FC<{ cheat: ProductivityCheat }> = ({ cheat }) => {
  const typeConfig = {
    revenue: { icon: DollarSign, color: "text-green-600 dark:text-green-400", bg: "bg-green-100 dark:bg-green-900/30" },
    efficiency: { icon: Zap, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-100 dark:bg-blue-900/30" },
    competition: { icon: Target, color: "text-red-600 dark:text-red-400", bg: "bg-red-100 dark:bg-red-900/30" },
    optimization: { icon: Brain, color: "text-purple-600 dark:text-purple-400", bg: "bg-purple-100 dark:bg-purple-900/30" }
  };
