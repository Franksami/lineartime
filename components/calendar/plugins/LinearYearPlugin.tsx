...
import { createPlugin, sliceEvents, EventSegment } from '@fullcalendar/core'
import { 
  getDaysInMonth, 
  startOfMonth, 
  endOfMonth, 
...
const LinearYearViewConfig = {
  ...
  willUnmount: function(props: any) {
...
function generateMonthsHtml(year: number, eventsByDate: Record<string, any[]>, overlappingDates: Set<string>): string {
  let monthsHtml = ''
    ...
    weeks.forEach((week) => {
      ...
      week.forEach((day) => {
        ...
        if (dayEvents.length > 0) {
          eventsHtml = dayEvents.slice(0, 4).map((event: any, index: number) => {
            ...
            return `
              <div class="event-dot absolute w-2 h-2 rounded-full transition-all hover:scale-125 ${
                hasOverlap ? 'bg-destructive border border-destructive-foreground animate-pulse' : 'bg-primary'
              }" 
        ...
        weekHtml += `
          <div class="day-cell relative h-12 flex items-center justify-center text-xs border border-border cursor-pointer transition-all hover:bg-accent hover:shadow-sm ${cn(
            isCurrentMonth 
              ? (isTodayDate ? 'text-foreground bg-primary/10 border-primary font-bold ring-1 ring-primary/30' : 'text-foreground bg-background') 
