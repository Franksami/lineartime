...
export interface CalendarPlugin {
  ...
  // Event hooks
  onEventCreate?: (event: Event) => Event | void
  onEventUpdate?: (event: Event, oldEvent: Event) => Event | void
  onEventDelete?: (event: Event) => boolean | void // return false to prevent deletion
...
export function PluginManagerProvider({
  ...
  const installPlugin = useCallback((plugin: CalendarPlugin) => {
  ...
  const uninstallPlugin = useCallback((pluginName: string) => {
  ...
  const activatePlugin = useCallback((pluginName: string) => {
    const plugin = installedPlugins.get(pluginName)
    if (!plugin) {
      notify.warning(`Plugin "${pluginName}" is not installed`)
      return
  ...
  const deactivatePlugin = useCallback((pluginName: string) => {
    const plugin = installedPlugins.get(pluginName)
    if (!plugin) {
      notify.warning(`Plugin "${pluginName}" is not installed`)
      return
  ...
  const isPluginActive = useCallback((pluginName: string) => {
...
export function usePluginEventHooks() {
  ...
  const executeEventHook = useCallback((hookName: keyof CalendarPlugin, ...args: any[]) => {
    const results: any[] = []
  ...
  return {
    ...
    onEventUpdate: (event: Event, oldEvent: Event) => executeEventHook('onEventUpdate', event, oldEvent),
