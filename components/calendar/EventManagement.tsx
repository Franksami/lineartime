...
import { announceToScreenReader, getEventAriaLabel } from '@/lib/accessibility';
...
export function EventManagement({ userId }: { userId: string }) {
  ...
  const [selectedDate, _setSelectedDate] = React.useState<Date | null>(null);
  ...
  const filteredEvents = React.useMemo(() => {
    ...
    switch (dateFilter) {
      case 'today':
        filtered = filtered.filter(
          (event) => event.startDate >= startOfDay(now) && event.startDate <= endOfDay(now)
        );
        break;
      case 'week':
        filtered = filtered.filter(
          (event) => event.startDate >= startOfWeek(now) && event.startDate <= endOfWeek(now)
        );
        break;
      case 'month':
        filtered = filtered.filter(
          (event) => event.startDate >= startOfMonth(now) && event.startDate <= endOfMonth(now)
        );
        break;
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        const _newOrder = arrayMove(filteredEvents, oldIndex, newIndex);
  ...
  const handleSaveEvent = async (eventData: Partial<Event>) => {
    if (selectedEvent) {
      ...
      const id = events?.find(
        (e) => e.convexId === selectedEvent.id || String(e.id) === selectedEvent.id
      )?.id;
      if (id) {
