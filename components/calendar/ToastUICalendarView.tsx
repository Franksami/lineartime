...
export function ToastUICalendarView({
  ...
  const toastUIEvents = useMemo(() => {
    return events.map((event): ToastUIEvent => ({
  ...
  const handleBeforeUpdateEvent = useCallback(({ event, changes }: any) => {
  ...
  const handleBeforeDeleteEvent = useCallback((eventData: any) => {
  ...
  const stats = useMemo(() => {
    ...
    const completedEvents = events.filter(event => 
      event.completed && new Date(event.start) <= new Date()
    ).length;
  ...
  const formatCurrentDate = useCallback(() => {
    const options: Intl.DateTimeFormatOptions = 
      currentView === 'month' ? { month: 'long', year: 'numeric' } :
      currentView === 'week' ? { month: 'short', day: 'numeric', year: 'numeric' } :
              ...
              <div className="h-full flex flex-col p-4 space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    {calendars.map((cal) => (
                      <div key={cal.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                        <div 
