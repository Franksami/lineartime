...
export function DayDetailView({
  ...
  const handleEventResize = (eventId: string, _width: number, height: number) => {
      ...
      <div className="fixed inset-4 md:inset-8 bg-background border border-border rounded-lg shadow-sm flex flex-col">
        ...
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setIsDragEnabled(!isDragEnabled)}>
        ...
        <div className="flex-1 overflow-auto p-4">
          <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <div ref={containerRef} className="relative min-h-[1440px]">
              {' '}
              {/* 24 hours * 60px */}
              {/* Hour grid */}
            ...
            <DragOverlay>
              {activeEvent && (
                ...
                >
                  <DraggableEvent event={activeEvent} dragDisabled={true} />
                </div>
              )}
            </DragOverlay>
          </DndContext>
