...
import { ArrowRightLeft, Calendar as CalendarIcon, Shuffle, Target, Users } from 'lucide-react';
...
export function CrossCalendarDragView({
  ...
  const sharedEvents = useMemo(
    () =>
      convertEventsToFullCalendar(
        [
          ...eventDistribution.source,
          ...eventDistribution.destination,
          ...eventDistribution.shared,
        ],
        'shared'
      ),
  ...
  const handleEventReceive = useCallback(
    (info: any, targetCalendarId: string) => {
      ...
      setEventDistribution((prev) => {
        ...
        if (sourceCalendarId && sourceCalendarId !== targetCalendarId) {
          newDistribution[sourceCalendarId as keyof typeof newDistribution] = newDistribution[
            sourceCalendarId as keyof typeof newDistribution
          ].filter((e) => e.id !== originalEvent.id);
        }

        // Add to destination calendar (avoid duplicates)
        if (
          !newDistribution[targetCalendarId as keyof typeof newDistribution].some(
      ...
      notify.success(
        `Event "${originalEvent.title}" moved to ${CALENDAR_CONFIGS[targetCalendarId as keyof typeof CALENDAR_CONFIGS].title}`
      );
    },
    [onCrossCalendarDrag]
  );
  ...
  const handleEventLeave = useCallback((info: any, sourceCalendarId: string) => {
    ...
    notify.info(
      `Event "${originalEvent.title}" being moved from ${CALENDAR_CONFIGS[sourceCalendarId as keyof typeof CALENDAR_CONFIGS].title}`
    );
  }, []);
  ...
  const handleDateSelect = useCallback(
    (selectInfo: DateSelectArg, calendarId: string) => {
      ...
      notify.success(
        `Event created on ${format(start, 'MMM d, yyyy')} in ${CALENDAR_CONFIGS[calendarId as keyof typeof CALENDAR_CONFIGS].title}`
      );
    },
      ...
      <div className="flex items-center justify-between p-4 border-b border-border bg-card">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="secondary">
            {eventDistribution.source.length +
              eventDistribution.destination.length +
              eventDistribution.shared.length}{' '}
            events
          </Badge>
          <Button variant="outline" size="sm" onClick={handleShuffleEvents} className="gap-2">
                  ...
                  <Badge variant="outline" className="ml-auto">
                    {calendarId === 'shared'
                      ? eventDistribution.source.length +
                        eventDistribution.destination.length +
                        eventDistribution.shared.length
                      : eventDistribution[calendarId as keyof typeof eventDistribution].length}{' '}
                    events
                  </Badge>
                </CardTitle>
