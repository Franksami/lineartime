...
export function LinearCalendarPro({
  ...
  const { startRenderMeasurement, endRenderMeasurement, metrics } = usePerformanceMonitor(events.length)
  ...
  }, [events, deferredEvents, overlappingEvents, maxEvents, enableVirtualRendering, enableCollisionDetection, enableAdvancedDragDrop])
    ...
    startTransition(() => {
      ...
      if (enableCollisionDetection) {
        ...
        if (conflictingEvents.length > 0) {
          ...
          notify.warning(`Event moved but conflicts with ${conflictingEvents.length} other event(s)`)
        } else {
    ...
    startTransition(() => {
      ...
      if (enableCollisionDetection) {
        ...
        if (conflictingEvents.length > 0) {
          ...
          notify.warning(`Event resized but conflicts with ${conflictingEvents.length} other event(s)`)
        } else {
          setOverlappingEvents(prev => {
            const newSet = new Set(prev)
            newSet.delete(event.id)
  ...
  // Handle view changes
  const handleViewDidMount = useCallback((info: { view: ViewApi }) => {
    const { view } = info
  ...
  const handleDatesSet = useCallback((info: { start: Date, end: Date, view: ViewApi }) => {
  ...
  return (
    <div className={cn(
      ...
      <FullCalendar
        ...
        eventConstraint={enableCollisionDetection ? {
          start: '00:00',
          end: '24:00'
        } : undefined}
...
function getCategoryColor(category: string, isOverlapping: boolean = false): string {
  const colors = {
