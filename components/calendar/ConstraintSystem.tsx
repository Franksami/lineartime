...
import { format, differenceInMinutes, isSameDay, isAfter, isBefore, isWeekend, getHours, startOfDay, endOfDay } from 'date-fns'
...
const DEFAULT_BUSINESS_RULES: BusinessRule[] = [
  {
    ...
    autoFix: (event) => ({
      ...event,
      startDate: new Date(),
      endDate: event.endDate && isBefore(event.endDate, new Date()) 
  ...
  
  {
    ...
    validate: (event, existingEvents) => {
      ...
      const conflicts = existingEvents.filter(existing => {
        ...
        return (
          (isAfter(event.startDate!, existing.startDate) && isBefore(event.startDate!, existing.endDate)) ||
          (isAfter(event.endDate!, existing.startDate) && isBefore(event.endDate!, existing.endDate)) ||
          (isBefore(event.startDate!, existing.startDate) && isAfter(event.endDate!, existing.endDate)) ||
      ...
      return {
        ...
        details: conflicts.length > 0 
          ? conflicts.map(c => `• ${c.title} (${format(c.startDate, 'p')} - ${format(c.endDate, 'p')})`).join('\n')
          : undefined,
...

  {
    ...
    validate: (event) => {
      ...
      return {
        ...
        suggestedFix: duration > maxDuration ? 'Consider splitting into multiple events' : undefined
      }
    }
  },

  {
    ...
    validate: (event) => {
      ...
      return {
        ...
        details: isWorkHours ? undefined : 'Consider scheduling between 9 AM - 5 PM for better productivity'
      }
    }
  },
...

  {
    ...
    validate: (event) => {
      ...
      const isValid = isAfter(event.endDate, event.startDate) || 
...

  {
    ...
    validate: (event, existingEvents) => {
      ...
      const sameDay = existingEvents.filter(existing => 
        existing.id !== event.id && isSameDay(existing.startDate, event.startDate!)
      )

      // Calculate total duration for the day
      const eventDuration = differenceInMinutes(event.endDate, event.startDate)
      const existingDuration = sameDay.reduce((total, existing) => 
        total + differenceInMinutes(existing.endDate, existing.startDate), 0
      )
...
export function ConstraintSystem({
  ...
  const groupedResults = useMemo(() => {
    return validationResults.reduce((acc, { rule, result }) => {
  ...
  // Handle auto-fix
  const handleAutoFix = useCallback((rule: BusinessRule) => {
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {overallStatus === 'error' ? 'Issues Found' : 
             overallStatus === 'warning' ? 'Warnings' : 'Valid'}
          </Badge>
          <span className="text-sm text-muted-foreground">
            {validationResults.length} constraint{validationResults.length !== 1 ? 's' : ''} evaluated
          </span>
        </div>
      )}
      ...
      <div className="space-y-2">
        {Object.entries(groupedResults).map(([severity, results]) => {
          ...
          return results.map(({ rule, result }, index) => {
            ...
            return (
              <Alert key={`${severity}-${index}`} className={`${config.bgColor} ${config.borderColor}`}>
