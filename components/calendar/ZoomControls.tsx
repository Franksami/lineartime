...
export function ZoomControls({ 
  ...
  const currentZoomLevel = zoomLevel || numericZoomLevel
  ...
  const renderLegacyContent = () => (
    ...
    >
      ...
      >
        {typeof numericZoomLevel === 'number' ? Math.round(numericZoomLevel * 100) + '%' : zoomLevel}
      </Button>
  ...
  React.useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isMobileMenuOpen && !target.closest('[role="menu"]') && !target.closest('button[aria-expanded="true"]')) {
        setIsMobileMenuOpen(false)
      }
    }
