...
import { Plus, Minus, Move, Home } from 'lucide-react'
    ...
    if (over && active.id !== over.id) {
      ...
      if (draggedEvent && over.data.current) {
        ...
        if (targetData.date) {
          ...
          const daysDiff = Math.floor((targetDate.getTime() - draggedEvent.startDate.getTime()) / (1000 * 60 * 60 * 24))
          
          const updatedEvent = {
            ...draggedEvent,
            startDate: addDays(draggedEvent.startDate, daysDiff),
            endDate: draggedEvent.endDate ? addDays(draggedEvent.endDate, daysDiff) : addDays(draggedEvent.startDate, daysDiff)
          }
          ...
          if (updateEvent) {
            ...
            if (dbEvent) {
              await updateEvent(parseInt(dbEvent.id) || 0, {
        ...
        <div className="absolute inset-0 pointer-events-none">
          {events.map((event, idx) => {
            ...
            const eventDuration = event.endDate ? 
              Math.ceil((event.endDate.getTime() - event.startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1 : 1
            const eventWidth = Math.min(eventDuration * cellWidth - 4, (42 - dayPosition) * cellWidth - 4)
                  ...
                  if (updateEvent) {
                    ...
                    if (dbEvent) {
                      await updateEvent(parseInt(dbEvent.id) || 0, {
                        ...
                        endTime: resizedEvent.endDate?.getTime() || resizedEvent.startDate.getTime(),
                      })
                    }
                  }
                  // Call parent update handler
        ...
        <FloatingToolbar
          ...
          onDelete={async (eventId) => {
            ...
            if (deleteEvent) {
              ...
              if (dbEvent) {
                await deleteEvent(parseInt(dbEvent.id) || 0)
              }
            }
          ...
          onDuplicate={async (event) => {
            ...
            if (onEventCreate) {
              ...
              await createEvent({
                ...
                endTime: duplicatedEvent.endDate?.getTime() || duplicatedEvent.startDate!.getTime(),
