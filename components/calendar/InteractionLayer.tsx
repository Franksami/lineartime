  ...
  const handleKeyDown = React.useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (!focusedDate && (e.key === 'Tab' || e.key === 'Enter')) {
        ...
        setAnnounceMessage(
          `Entered calendar navigation mode. Current date: ${format(today, 'EEEE, MMMM d, yyyy')}`
        );
        return;
      }
      ...
      if (handled) {
        ...
        if (newDate !== focusedDate) {
          ...
          if (scrollRef.current) {
            const dayOfYear =
              Math.floor(
                (newDate.getTime() - new Date(year, 0, 1).getTime()) / (1000 * 60 * 60 * 24)
              ) + 1;
      ...
      {keyboardMode &&
        ...
        (() => {
          ...
          const dayOfYear = Math.floor(
            (focusedDate.getTime() - yearStartDate.getTime()) / MS_PER_DAY
          );
