                ...
                <div className="flex items-center gap-2">
                  <div
                    className={cn(
                      ...
                      priority === 'critical' && 'bg-red-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */',
                      priority === 'high' && 'bg-orange-500',
                      priority === 'medium' && 'bg-yellow-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */',
                      priority === 'low' && 'bg-green-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */',
                      priority === 'optional' && 'bg-gray-400 /* TODO: Use semantic token */ /* TODO: Use semantic token */'
                    )}
                  />
            ...
            <ContextMenuItem>
              <div className="flex items-center gap-2 w-full">
                ...
                {event.syncStatus?.google === 'synced' && (
                  <CheckCircle className="h-3 w-3 text-green-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */ ml-auto" />
                )}
                {event.syncStatus?.google === 'pending' && (
                  <Clock className="h-3 w-3 text-yellow-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */ ml-auto" />
                )}
                {event.syncStatus?.google === 'error' && (
                  <XCircle className="h-3 w-3 text-red-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */ ml-auto" />
                )}
              </div>
            </ContextMenuItem>
            <ContextMenuItem>
      ...
      <CardContent className="p-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <Button variant="outline" size="sm" onClick={() => onBulkOperation('duplicate')}>
              <Copy className="h-4 w-4 mr-1" />
              Duplicate
            </Button>

            <Button variant="outline" size="sm" onClick={() => onBulkOperation('archive')}>
  ...
  return (
    <Card className="mb-4">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Category Filters */}
        <div>
          <label className="text-xs font-medium text-muted-foreground mb-2 block">Categories</label>
          <div className="flex flex-wrap gap-2">
            {['personal', 'work', 'effort', 'note', 'meeting', 'deadline', 'milestone'].map(
              (category) => (
...
export function EnhancedEventManagement({
  ...
  const managedEvents: ManagedEvent[] = useMemo(
    () => events.map((event) => ({ ...event, isSelected: false })),
    [events]
  );
  ...
  const _performanceMonitor = usePerformanceMonitor(managedEvents.length);
  ...
  const handleBulkOperation = useCallback(
    (operation: BulkOperation, data?: any) => {
      ...
      notify.success(
        `${operation} applied to ${selectedIds.length} event${selectedIds.length !== 1 ? 's' : ''}`
      );
    },
  ...
  return (
    <div className={cn('enhanced-event-management', className)}>
      <EventFilters context={context} onContextChange={handleContextChange} />
      ...
      <div className="space-y-2">
        {managedEvents.map((event) => (
          <EventContextMenu
            ...
            onChangeCategory={(_id, category) => notify.info(`Changed category to: ${category}`)}
            onChangePriority={(_id, priority) => notify.info(`Changed priority to: ${priority}`)}
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {event.isPinned && (
                    <Pin className="h-4 w-4 text-yellow-500 /* TODO: Use semantic token */ /* TODO: Use semantic token */" />
                  )}
                </div>
              </div>
            </Card>
