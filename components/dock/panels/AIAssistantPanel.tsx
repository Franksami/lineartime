...
function useAIAssistantPanel() {
  const [activeAgent, setActiveAgent] = useState<'planner' | 'conflict' | 'summarizer' | 'router'>('planner')
    ...
    try {
      ...
      switch (activeAgent) {
        case 'planner':
          ...
          if (input.toLowerCase().includes('optimize') || input.toLowerCase().includes('schedule')) {
            // TODO: Get actual events/tasks from workspace
            const mockEvents: any[] = []
            const mockTasks: any[] = []
            response = await plannerAgent.optimizeSchedule(mockEvents, mockTasks)
            addMessage('planner', 'response', `Found ${response.violations.length} violations, generated ${response.solutions.length} solutions`)
          } else {
            addMessage('planner', 'response', 'I can help with scheduling optimization and conflict resolution. Try "optimize my schedule" or "find conflicts".')
          }
          break
        ...
        case 'router':
          ...
          addMessage('router', 'response', `Intent: ${response.classification.intent} (${(response.classification.confidence * 100).toFixed(0)}% confidence)`)
          break
      }
    ...
    <div className="space-y-3">
      ...
      <Card className="bg-green-50/50">
        <CardContent className="p-3">
          ...
          <p className="text-xs text-muted-foreground">
            Content summarization and conversation
          </p>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge variant="outline">
              {toolSafety.safetyStatus.successRate} Success
            </Badge>
          </div>
          <p className="text-xs text-muted-foreground">
            {toolSafety.safetyStatus.registeredTools} tools • {toolSafety.safetyStatus.autoApprovalTools} auto-approved
          </p>
        </CardContent>
      </Card>
    </div>
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-4">
          ...
          <Badge variant="outline" className="text-xs">ai.plannerAgent</Badge>
        </div>
      </div>
    )
  }
                ...
                <Card>
                  <CardContent className="p-3">
                    <div className="space-y-2">
                      ...
                      <div className="text-xs text-muted-foreground">
                        • {agents.planner.lastResults.executionTime?.toFixed(0) || 0}ms execution time
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <TabsContent value="conflict" className="flex-1 m-0 p-3">
            <div className="h-full space-y-4">
              ...
              {agents.conflict.conflicts.length > 0 && (
                <div className="space-y-2">
                  <div className="text-xs font-medium">Detected Conflicts ({agents.conflict.conflicts.length})</div>
