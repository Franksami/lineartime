...
export class WorkflowEngine {
  ...
  private static async executeWorkflowStep(
    ...
    try {
      ...
      run.progress.percentage = Math.round((run.progress.completedSteps / run.progress.totalSteps) * 100)
  ...
  private static async executeDataCollectionStep(step: WorkflowStep, run: WorkflowRun): Promise<void> {
   ...
   */
  static async skipStep(runId: string, stepId: string, skippedBy: string, reason?: string): Promise<boolean> {
    const run = this.activeRuns.get(runId)
    const workflow = this.workflows.get(run?.workflowId || '')
   ...
   */
  private static evaluateCondition(condition: WorkflowStep['condition'], run: WorkflowRun): boolean {
  ...
  static getEngineStatus() {
    return {
      ...
      // Performance metrics
      performance: {
        averageExecutionTime: this.runHistory.length > 0
          ? this.runHistory.reduce((sum, run) => {
              if (run.startedAt && run.completedAt) {
                return sum + (new Date(run.completedAt).getTime() - new Date(run.startedAt).getTime())
              }
              return sum
            }, 0) / this.runHistory.filter(r => r.completedAt).length
          : 0,
        successRate: this.runHistory.length > 0
          ? (this.runHistory.filter(r => r.status === 'completed').length / this.runHistory.length * 100)
          : 100
      },
