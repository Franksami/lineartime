...
export class OfflineSyncManager {
  ...
  async performSync(): Promise<SyncResult> {
    ...
    try {
      ...
      const nonConflictedLocal = localPendingEvents.filter(e => 
        !conflicts.some(c => c.eventId === e.id)
      )
      const nonConflictedRemote = remoteEvents.filter(e => 
        !conflicts.some(c => c.eventId === e.id)
      )
      ...
      const syncResults = await this.performBatchSync([
        ...nonConflictedLocal,
        ...resolutions.map(r => r.resolvedEvent)
      ], nonConflictedRemote)
  ...
  private detectConflicts(localEvents: SyncableEvent[], remoteEvents: SyncableEvent[]): EventConflict[] {
  ...
  private determineConflictType(local: SyncableEvent, remote: SyncableEvent): EventConflict['conflictType'] | null {
  ...
  private suggestResolution(local: SyncableEvent, remote: SyncableEvent, conflictType: EventConflict['conflictType']): ConflictResolution {
  ...
  private mergeEvents(local: SyncableEvent, remote: SyncableEvent): SyncableEvent {
    const merged: SyncableEvent = {
      ...
      startDate: this.mergeTime(local.startDate, remote.startDate, local.lastModified, remote.lastModified),
      endDate: this.mergeTime(local.endDate, remote.endDate, local.lastModified, remote.lastModified),
  ...
  private mergeTitle(local: SyncableEvent, remote: SyncableEvent): string {
    ...
    switch (titlePriority) {
      case 'local': return local.title
      case 'remote': return remote.title
      case 'longest': return local.title.length > remote.title.length ? local.title : remote.title
      case 'newest':
      default:
  ...
  private mergeTime(localTime: string, remoteTime: string, localModified: number, remoteModified: number): string {
    const { timePriority } = this.config.mergePreferences
    
    switch (timePriority) {
      case 'local': return localTime
      case 'remote': return remoteTime
      case 'earliest': return new Date(localTime) < new Date(remoteTime) ? localTime : remoteTime
      case 'latest': return new Date(localTime) > new Date(remoteTime) ? localTime : remoteTime
      default:
  ...
  private mergeDescription(local: SyncableEvent, remote: SyncableEvent): string | undefined {
    ...
    switch (descriptionPriority) {
      case 'local': return local.description
      case 'remote': return remote.description
      case 'longest': return localDesc.length > remoteDesc.length ? local.description : remote.description
      case 'merge': 
  ...
  private mergeTags(local: SyncableEvent, remote: SyncableEvent): string[] | undefined {
    ...
    switch (tagsMergeStrategy) {
      case 'local': return local.tags
      case 'remote': return remote.tags
      case 'union':
      default:
  ...
  private async getRemoteEventsSinceLastSync(): Promise<SyncableEvent[]> {
    try {
      const response = await fetch(`/api/events/sync?since=${this.lastSyncTime}&deviceId=${this.deviceId}`)
  ...
  private async performBatchSync(localEvents: SyncableEvent[], remoteEvents: SyncableEvent[]): Promise<{
    syncedCount: number
    errorCount: number
    errors: SyncError[]
  }> {
  ...
  public async resolveConflictManually(eventId: string, resolution: ConflictResolution): Promise<void> {
