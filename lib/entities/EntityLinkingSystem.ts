...
export class EntityLinkingSystem {
  ...
  static async createLink(
    ...
    console.log(
      `🔗 Creating link: ${fromEntity.type}/${fromEntity.id} → ${toEntity.type}/${toEntity.id}`
    );
  ...
  static async handleDragToLink(
    ...
    const linkType = this.determineLinkTypeFromContext(
      draggedEntityId,
      targetEntityId,
      dropContext
    );
  ...
  static getEntityGraph(
    ...
    const nodes = filteredEntities.map((entity) => {
      const connectionCount = filteredLinks.filter(
        (link) => link.from.id === entity.id || link.to.id === entity.id
      ).length;
  ...
  private static invalidateBacklinkCache(entityId: string): void {
    ...
    const relatedLinks = Array.from(this.links.values()).filter(
      (link) => link.from.id === entityId || link.to.id === entityId
    );
  ...
  static getLinkingStats() {
    ...
    const entityTypeCount = Array.from(this.entities.values()).reduce(
      (acc, entity) => {
    ...
    const linkTypeCount = Array.from(this.links.values()).reduce(
      (acc, link) => {
