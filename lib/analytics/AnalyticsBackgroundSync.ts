...
export class AnalyticsBackgroundSync {
  ...
  public queueDragDropEvent(event: DragDropEvent, priority: 'low' | 'medium' | 'high' = 'medium'): void {
  ...
  public queueAnalyticsBatch(metrics: DragDropMetrics, priority: 'low' | 'medium' | 'high' = 'low'): void {
    const item: AnalyticsQueueItem = {
      id: `batch-${Date.now()}`,
  ...
  private addToQueue(item: AnalyticsQueueItem): void {
    ...
    if (this.syncQueue.length >= this.config.maxQueueSize) {
      ...
      this.syncQueue = this.syncQueue.filter(item => item.priority !== 'low').slice(0, this.config.maxQueueSize - 1)
    }
  ...
  private async syncPriorityQueue(priority: 'low' | 'medium' | 'high'): Promise<void> {
    ...
    try {
      ...
      this.syncQueue = this.syncQueue.filter(item => 
        !itemsToSync.some(synced => synced.id === item.id)
      )
  ...
  public getQueueStatus(): {
    ...
    const byPriority = this.syncQueue.reduce((acc, item) => {
      acc[item.priority]++
      return acc
  ...
  public updateConfig(newConfig: Partial<BackgroundSyncConfig>): void {
    ...
    if (newConfig.highPriorityInterval || newConfig.mediumPriorityInterval || newConfig.lowPriorityInterval) {
