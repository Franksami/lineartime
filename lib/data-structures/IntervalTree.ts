...
export class IntervalTree {
  ...
  findOverlapping(start: Date, end: Date): Event[] {
    const results: Event[] = [];
    this.searchOverlapping(this.root, start.getTime(), end.getTime(), results);
    return results;
  }
  ...
  private insertNode(node: IntervalNode | null, interval: TimeInterval): IntervalNode {
    ...
    node.max = Math.max(
      node.interval.end,
      Math.max(node.left ? node.left.max : 0, node.right ? node.right.max : 0)
    );
  ...
  private rotateRight(y: IntervalNode): IntervalNode {
    ...
    // Update max values
    y.max = Math.max(y.interval.end, Math.max(y.left ? y.left.max : 0, y.right ? y.right.max : 0));

    x.max = Math.max(x.interval.end, Math.max(x.left ? x.left.max : 0, x.right ? x.right.max : 0));
