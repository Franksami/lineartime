...
export class CalendarRenderer {
  ...
  private getEventRenderObject(event: Event, month: number): EventRenderObject | null {
    ...
    const duration = Math.ceil((event.endDate.getTime() - event.startDate.getTime()) / (1000 * 60 * 60 * 24)) + 1
    const width = Math.min(duration * this.options.cellWidth - this.options.padding * 2, 7 * this.options.cellWidth)
  ...
  public handleInteraction(x: number, y: number, type: 'hover' | 'click'): Event | null {
    ...
    for (const [id, renderObj] of this.activeEvents) {
      if (x >= renderObj.x && x <= renderObj.x + renderObj.width &&
          y >= renderObj.y && y <= renderObj.y + renderObj.height) {
