...
Respond with valid JSON only:`.replace("{input}", preprocessedInput),
      ...
      // Handle time parsing
      if (object.startTime) {
        const [hours, minutes] = this.parseTime(object.startTime)
        startDate.setHours(hours, minutes)
      }

      if (object.endTime) {
        const [hours, minutes] = this.parseTime(object.endTime)
  ...
  static async getSchedulingSuggestions(
    ...
    try {
      ...
      const conflicts = this.detectConflicts(events, newEvent)
  ...
  static async chatWithAssistant(message: string, context: CalendarEvent[]) {
    try {
      const { text } = await generateText({
        ...
        prompt: AI_PROMPTS.CHAT_ASSISTANT.replace("{context}", JSON.stringify(context)).replace("{message}", message),
      })
