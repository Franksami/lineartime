...
const createComponentRegistry = (): ComponentRegistry => ({
  shadcn: {
    ...
    Sidebar: {
      component: lazy(() => import('@/components/ui/sidebar').then(m => ({ default: m.Sidebar }))),
      lazy: true
    },
    CommandPalette: {
      component: lazy(() => import('@/components/ui/command').then(m => ({ default: m.Command }))),
      lazy: true
    }
  },
...
export function useUIAdapter() {
  ...
  const switchLibrary = React.useCallback((library: UILibrary) => {
  ...
  const getComponent = React.useCallback((
    ...
    library: UILibrary = currentLibrary
  ) => {
    ...
    try {
      ...
      if (!libraryRegistry || !libraryRegistry[componentName]) {
        console.warn(`⚠️ Component ${componentName} not found in ${library}, falling back to ${fallbackLibrary}`)
...
export const migrationUtils = {
  ...
  validateMigration: async (componentName: AdaptableComponent, fromLibrary: UILibrary, toLibrary: UILibrary) => {
