...
export class EventParser {
  ...
  private registerCustomRefiners() {
    ...
    this.parser.refiners.push({
      refine: (context, results) => {
        const recurringPattern = /(?:every|each)\s+(day|week|month|year|monday|tuesday|wednesday|thursday|friday|saturday|sunday)/i;
  ...
  private generateTitle(input: string, temporal: any, location?: string): string {
  ...
  private inferCategory(input: string): EventCategory {
    const workKeywords = ['meeting', 'call', 'presentation', 'review', 'standup', 'work', 'office', 'deadline', 'project', 'client', 'interview'];
    const personalKeywords = ['birthday', 'dinner', 'lunch', 'coffee', 'gym', 'doctor', 'appointment', 'date', 'party', 'wedding', 'vacation'];
    const effortKeywords = ['workout', 'study', 'practice', 'work on', 'build', 'learn', 'training', 'exercise', 'run', 'coding'];
  ...
  public parseIntent(input: string): EventIntent | null {
    ...
    // Update intent
    if (lower.includes('update') || lower.includes('change') || lower.includes('modify') || lower.includes('reschedule')) {
      const target = input.replace(/^(update|change|modify|reschedule)\s+/i, '').trim();
      return { action: 'update', target };
    }
    
    // Search intent
    if (lower.includes('find') || lower.includes('search') || lower.includes('show') || lower.includes('list')) {
