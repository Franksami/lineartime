...
import { addDays, format, startOfWeek, endOfWeek, isToday, isFuture } from 'date-fns'
...
export class CalendarAI {
   ...
   */
  async processRequest(userInput: string, context?: {
    currentDate?: Date
    ...
    response: string
    actions: Array<{
      type: 'create_event' | 'suggest_times' | 'find_conflicts' | 'reschedule' | 'analyze_productivity'
      data: any
      confidence: number
    }>
    suggestions: string[]
...
Be proactive about long-term planning and productivity optimization. Consider work-life balance and suggest improvements beyond just scheduling.`
      ...
      return {
        response: "I apologize, but I'm having trouble processing your request right now. Could you try rephrasing it or ask me something specific about your calendar?",
        actions: [],
        suggestions: [
...
Focus on long-term productivity and "life is bigger than a week" philosophy.`
      ...
      const processedInsights = insights.patterns.map(pattern => ({
        type: pattern.impact === 'positive' ? 'productivity' as const : 
              pattern.impact === 'negative' ? 'optimization' as const : 'scheduling' as const,
  ...
  private async analyzeIntent(userInput: string, context: string): Promise<{
    intent: string
    confidence: number
    ...
    if (lowerInput.includes('schedule') || lowerInput.includes('meeting') || lowerInput.includes('appointment')) {
      return {
        ...
        followUp: ['Would you like me to check for conflicts?', 'Should I invite anyone to this meeting?']
      }
    }
    
    if (lowerInput.includes('available') || lowerInput.includes('free time') || lowerInput.includes('open')) {
      return {
    ...
    if (lowerInput.includes('conflict') || lowerInput.includes('busy') || lowerInput.includes('double book')) {
      return {
    ...
    if (lowerInput.includes('productivity') || lowerInput.includes('insights') || lowerInput.includes('analysis')) {
      return {
  ...
  private async executeActions(actions: Array<{ type: string; parameters: any }>, originalInput: string): Promise<any[]> {
    const results = []
