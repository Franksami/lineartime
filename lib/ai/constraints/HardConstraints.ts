import { TimeSlot, SchedulingConstraint, SchedulingContext } from '../types';
import { Event } from '@/types/calendar';
import { isWithinInterval, isWeekend, getDay } from 'date-fns';
...
export const WorkingHoursConstraint: SchedulingConstraint = {
  ...
  evaluate: (slot: TimeSlot, context: SchedulingContext): boolean => {
    ...
    return slotStartHour >= workingHours.start && 
           slotEndHour <= workingHours.end;
  }
};
...
export const AttendeesAvailabilityConstraint = (
  ...
  evaluate: (slot: TimeSlot): boolean => {
    ...
    return attendeeEvents.every(events => 
      !events.some(event => {
        const eventStart = event.startDate.getTime();
        const eventEnd = event.endDate.getTime();
        const slotStart = slot.start.getTime();
...
export const LocationAvailabilityConstraint = (
...
export const RecurringEventConstraint = (
  recurringPattern: { daysOfWeek?: number[]; dates?: Date[] }
): SchedulingConstraint => ({
  type: 'hard',
