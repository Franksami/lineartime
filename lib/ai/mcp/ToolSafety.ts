...
export class ToolSafetyManager {
  ...
  private static executionCounts: Map<string, { userId: string, count: number, hourStart: number }> = new Map()
    ...
    this.registerToolSafety('calendar.resolveConflicts', {
      ...
      preExecutionChecks: [
        async (context, params) => {
          ...
          return context.userRole === 'editor' || context.userRole === 'admin' || context.userRole === 'owner'
        }
      ],
      postExecutionChecks: [
        async (context, result) => {
  ...
  static async executeToolSafely(
    ...
    try {
      ...
      const paramValidation = await this.validateParameters(parameters, safetyConfig.parameterRules)
      ...
      const rateLimitOk = this.checkExecutionLimits(toolName, context.userId, safetyConfig.maxExecutionsPerHour)
      if (!rateLimitOk) {
        throw new Error('Rate limit exceeded for tool execution')
      }
      
      // Step 3: Validate scope permissions  
      const scopeValid = safetyConfig.allowedScopes.includes(context.activeView) || 
      ...
      const requiresConfirmation = safetyConfig.permissionLevel === ToolPermissionLevel.CONFIRM ||
   ...
   */
  private static checkExecutionLimits(toolName: string, userId: string, maxPerHour: number): boolean {
  ...
  private static createAuditLog(
    ...
    return {
      ...
      execution: {
        ...
        permissionLevel: this.safetyConfigs.get(toolName)?.permissionLevel || ToolPermissionLevel.MANUAL_ONLY,
  ...
  static clearAuditLogs(userId?: string, olderThanDays?: number) {
    if (userId && olderThanDays) {
      ...
      this.auditLogs = this.auditLogs.filter(log => 
        log.userId !== userId || new Date(log.timestamp) > cutoffDate
      )
    } else if (userId) {
  ...
  static getSafetyStatus() {
    const autoApprovalCount = Array.from(this.safetyConfigs.values())
      .filter(config => config.permissionLevel === ToolPermissionLevel.AUTO_APPROVE).length
    ...
    return {
      ...
      averageExecutionTime: totalExecutions > 0 
        ? this.auditLogs.reduce((sum, log) => sum + log.execution.executionTime, 0) / totalExecutions
        : 0,
...
export function useToolSafety() {
  ...
  const executeToolSafely = async (toolName: string, parameters: any, context: any, options?: any) => {
    const result = await ToolSafetyManager.executeToolSafely(toolName, parameters, context, options)
