...
import { tool } from 'ai';
import { addDays, addMinutes, endOfDay, format, isAfter, isBefore, startOfDay } from 'date-fns';
import { z } from 'zod';
...
export const findConflictsSchema = z
  .object({
...
export const suggestTimesSchema = z
  .object({
    ...
    preferences: z.object({
      ...
      preferredDays: z
        .array(z.number())
        .optional()
        .describe('Preferred days of week (0=Sunday, 6=Saturday)'),
    }),
    attendees: z
      .array(z.string())
      .optional()
      .describe('Attendee email addresses for availability check'),
    existingEvents: z.array(z.any()).optional().describe('Current calendar events for context'),
  })
  .describe(
    'Use AI to suggest optimal scheduling times based on preferences and calendar analysis'
  );
...
export const rescheduleEventSchema = z
  .object({
    ...
    findAlternatives: z
      .boolean()
      .default(true)
      .describe('Find alternative times if specified time conflicts'),
    notifyAttendees: z
      .boolean()
      .default(true)
      .describe('Whether to notify attendees of the change'),
  })
  .describe(
    'Reschedule an existing event with intelligent conflict detection and alternative suggestions'
  );
...
export const getAvailabilityTool = tool({
  ...
  execute: async ({ startDate, endDate, duration, preferences = {} }) => {
    try {
      ...
      const availableSlots = finder.findAvailableSlots(start, end, duration, {
      ...
      const formattedSlots = filteredSlots.slice(0, 10).map((slot) => ({
        ...
        timeOfDay:
          slot.start.getHours() < 12
            ? 'morning'
            : slot.start.getHours() < 17
              ? 'afternoon'
              : 'evening',
      }));
  ...
  execute: async ({ eventId, date, startTime, endTime, includeBuffer = true }) => {
    try {
      ...
      if (eventId) {
        ...
        conflictsFound = intervalTree
          .findOverlapping(targetEvent.startDate, targetEvent.endDate)
      ...
      const conflictAnalysis = conflictsFound.map((conflict) => {
        const overlappingEvents = intervalTree
          .findOverlapping(new Date(conflict.startDate), new Date(conflict.endDate))
        ...
        return {
          ...
          duration: Math.round(
            (new Date(conflict.endDate).getTime() - new Date(conflict.startDate).getTime()) / 60000
          ),
          overlappingCount: overlappingEvents.length,
          severity:
            overlappingEvents.length > 2 ? 'high' : overlappingEvents.length > 0 ? 'medium' : 'low',
        };
      });
      ...
      return {
        ...
        summary: {
          ...
          severity:
            conflictsFound.length > 3 ? 'high' : conflictsFound.length > 1 ? 'medium' : 'low',
          recommendation:
    ...
    try {
      ...
      const suggestions = result.suggestions.map((suggestion) => ({
        ...
        aiInsights: {
          ...
          workloadBalance:
            suggestion.score > 85 ? 'optimal' : suggestion.score > 70 ? 'good' : 'busy',
          recommendationLevel:
            suggestion.score > 90
              ? 'highly recommended'
              : suggestion.score > 75
      ...
      return {
        ...
        analysis: {
          ...
          averageScore: Math.round(
            result.suggestions.reduce((acc, s) => acc + s.score, 0) / result.suggestions.length
          ),
          recommendation:
            suggestions.length > 0 ? 'Multiple good options available' : 'Limited availability',
        },
        alternativeOptions:
          result.alternativeOptions?.slice(0, 5).map((alt) => ({
    ...
    try {
      ...
      if (findAlternatives) {
        ...
        const duration = Math.round(
          (targetEvent.endDate.getTime() - targetEvent.startDate.getTime()) / 60000
        );
        ...
        return {
          ...
          recommendation:
            alternatives.length > 0
              ? 'Use one of the suggested alternative times'
