import { TimeSlot, SchedulingContext, SchedulingConstraint } from '../types';
...
export class SlotScorer {
  ...
  private calculateTimingScore(slot: TimeSlot, priority: number): number {
    ...
    switch (priority) {
      case 1: idealHours = 4; break;    // Within 4 hours
      case 2: idealHours = 24; break;   // Within 1 day
      case 3: idealHours = 72; break;   // Within 3 days
      case 4: idealHours = 168; break;  // Within 1 week
      default: idealHours = 336; break; // Within 2 weeks
    }
    
    if (hoursFromNow < 0) {
      // Slot is in the past
      return 0;
    } else if (hoursFromNow <= idealHours) {
      ...
      return Math.max(0, 100 * Math.pow(decayRate, periods));
    }
  }
  ...
  private calculateBalanceScore(slot: TimeSlot, context: SchedulingContext): number {
    ...
    const dayMinutes = dayEvents.reduce((sum, event) =>
    ...
    const weekMinutes = weekEvents.reduce((sum, event) =>
      sum + differenceInMinutes(event.endDate, event.startDate), 0
    );
