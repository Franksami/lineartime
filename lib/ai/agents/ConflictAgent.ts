...
export class ConflictAgent {
  ...
  async detectConflicts(events: any[], tasks: any[]): Promise<ConflictDetectionResult[]> {
    ...
    console.log(`✅ Conflict detection completed: ${conflicts.length} conflicts (${detectionTime.toFixed(2)}ms)`)
      ...
      for (let j = i + 1; j < events.length; j++) {
        ...
        if (this.eventsOverlap(event1, event2)) {
          ...
          const overlapEnd = new Date(Math.min(
            new Date(event1.end).getTime(), 
            new Date(event2.end).getTime()
          ))
  ...
  async applyResolution(
    ...
    try {
      ...
      console.log(`✅ Conflict resolution applied: ${resolution.title} (${executionTime.toFixed(2)}ms)`)
  ...
  private assessBusinessImpact(event1: any, event2: any): 'low' | 'medium' | 'high' {
    ...
    const isExternalMeeting = event1.attendees?.some((a: any) => a.external) || 
  ...
  private isAutoResolvable(event1: any, event2: any): boolean {
    ...
    return !event1.external && !event2.external && 
           event1.flexible !== false && event2.flexible !== false
  }
  ...
  private async createSplitResolution(event1: any, event2: any): Promise<ConflictResolution> {
    const longerEvent = this.getEventDuration(event1) > this.getEventDuration(event2) ? event1 : event2
    ...
    return {
      ...
      preview: {
        ...
        afterState: { events: [/* split events */] }
      },
