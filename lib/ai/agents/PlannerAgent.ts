...
export class PlannerAgent {
  ...
  private async generateOptimizationSolutions(
    ...
    if (violationsByType.soft) {
      const preferenceSolutions = await this.generatePreferenceSolutions(violationsByType.soft, events, tasks)
  ...
  private shareResources(event1: any, event2: any): boolean {
    ...
    return event1.resourceId === event2.resourceId ||
           event1.attendees?.some((a1: any) => 
             event2.attendees?.some((a2: any) => a1.id === a2.id)
           )
  }
  ...
  private async createMoveSolution(event: any, conflictingEvent: any, type: string): Promise<OptimizationSolution> {
