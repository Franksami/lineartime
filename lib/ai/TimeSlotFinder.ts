...
import { TimeSlot, TimeRange, UserPreferences } from './types';
import { 
  addMinutes, 
  isAfter, 
  isBefore, 
...
export class TimeSlotFinder {
  ...
  public findNextAvailableSlot(
    ...
    if (options?.avoidTimes && options.avoidTimes.length > 0) {
      const filteredSlots = slots.filter(slot =>
        !this.isInAvoidedTime(slot, options.avoidTimes!)
      );
   ...
   */
  public getFreeTimeStats(startDate: Date, endDate: Date): {
    ...
    const freeTimePercentage = totalWorkingTime > 0 
      ? (totalFreeTime / totalWorkingTime) * 100 
      : 0;
  ...
  public findCommonAvailability(
    ...
    let slots = finder.findAvailableSlots(
      dateRange.start,
      dateRange.end,
      duration,
    ...
    if (options?.minAttendees && options.minAttendees < attendeeSchedules.length) {
      slots = this.filterByMinimumAttendees(
        slots,
        attendeeSchedules,
        options.minAttendees
      );
    }
  ...
  private getDayBoundaries(
    ...
    if (this.preferences?.workingHours.days &&
