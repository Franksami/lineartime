...
function calculateLayout(events: Event[]): LayoutResult[] {
  ...
  events.forEach((event) => {
    ...
    const _month = event.startDate.getMonth();
    ...
    const width = Math.min(duration * cellWidth - padding * 2, 7 * cellWidth - x);
    const height = cellHeight / 4;

    // Find conflicts
    const conflicts = intervalTree
      .findOverlapping(event.startDate, event.endDate)
...
function optimizePositions(_events: Event[], layouts: LayoutResult[]): LayoutResult[] {
