...
export class EventLayoutEngine {
  ...
  private layoutCollisionGroup(group: CollisionGroup): LayoutedEvent[] {
    ...
    group.events.forEach((event, _eventIndex) => {
   ...
   */
  private calculateExpandedWidth(event: Event, columns: Event[][], startColumn: number): number {
