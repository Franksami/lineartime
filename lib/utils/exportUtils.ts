import type { DragDropEvent, DragDropMetrics } from '@/lib/analytics/dragDropAnalytics';
...
export function exportEventsToCSV(events: Event[], filename = 'calendar-events.csv'): void {
  try {
...
function generateAnalyticsCSV(analytics: any, year: number): string {
  ...
  const rows = [
    ...
    [
      ...
      analytics.avgEventsPerMonth.toString(),
      'Average events scheduled per month',
    ],
    [
      'Most Productive Month',
      analytics.mostProductiveMonth.month,
      `${analytics.mostProductiveMonth.events} events`,
    ],
    [
      'Productivity Score',
      `${Math.round((analytics.totalEvents / 365) * 100)}%`,
      'Percentage of year with scheduled events',
    ],
    ['', '', ''], // Empty row separator
    ['Category Breakdown', '', ''],
...
${analytics.monthlyStats
  ...
  .map((month: any) => `${month.month}: ${month.events} events, ${month.days} days scheduled`)
  .join('\n')}
`.trim();
}
 ...
 */
export function exportDragDropMetricsToCSV(
  metrics: DragDropMetrics,
  filename = 'dragdrop-metrics.csv'
): void {
  try {
 ...
 */
export function exportDragDropEventsToCSV(
  events: DragDropEvent[],
  filename = 'dragdrop-events.csv'
): void {
  try {
...
export function exportDragDropToJSON(
  metrics: DragDropMetrics,
  events: DragDropEvent[],
  filename = 'dragdrop-analytics.json'
): void {
  try {
    const jsonData = {
      ...
      summary: {
        ...
        dateRange: {
          earliest:
            events.length > 0
              ? new Date(Math.min(...events.map((e) => e.timestamp.getTime()))).toISOString()
              : null,
          latest:
            events.length > 0
              ? new Date(Math.max(...events.map((e) => e.timestamp.getTime()))).toISOString()
              : null,
        },
      },
    };
...
export function generateDragDropReport(metrics: DragDropMetrics, events: DragDropEvent[]): string {
  ...
  const _successfulDrops = events.filter((e) => e.eventType === 'drop_success').length;
  const _cancelledDrags = events.filter((e) => e.eventType === 'drop_cancel').length;
 ...
 */
function generateDragDropRecommendations(
  metrics: DragDropMetrics,
  _events: DragDropEvent[]
): string {
  ...
  if (metrics.averageDragDuration > 5000) {
    recommendations.push(
      '• Long drag durations detected - practice quick event placement for better workflow'
    );
  }

  const mostActiveDay = Object.entries(metrics.dragsByDayOfWeek).sort(([, a], [, b]) => b - a)[0];
  if (mostActiveDay && mostActiveDay[1] > 0) {
    recommendations.push(
      `• Peak activity on ${mostActiveDay[0]} - consider batch scheduling on this day`
    );
  }
...
function generateDragDropMetricsCSV(metrics: DragDropMetrics): string {
  ...
  const rows = [
    [
      'Total Drag Operations',
      metrics.totalDragOperations.toString(),
      'Total number of drag operations initiated',
    ],
    [
      'Successful Drops',
      metrics.successfulDrops.toString(),
      'Number of successful drop operations',
    ],
    ['Cancelled Drags', metrics.cancelledDrags.toString(), 'Number of cancelled drag operations'],
    [
      'Success Rate',
      `${metrics.totalDragOperations > 0 ? Math.round((metrics.successfulDrops / metrics.totalDragOperations) * 100) : 0}%`,
      'Percentage of successful drag operations',
    ],
    ['Average Duration', `${metrics.averageDragDuration}ms`, 'Average time per drag operation'],
    [
      'Efficiency Score',
      `${metrics.dragEfficiencyScore}/100`,
      'Overall drag & drop efficiency rating',
    ],
    ...
    [
      ...
      'Percentage of drops that used AI suggestions',
    ],
    [
      'Conflict Resolution Rate',
      `${Math.round(metrics.conflictResolutionRate * 100)}%`,
      'Percentage of drops that had conflicts resolved',
    ],
    [
      'Optimization Acceptance',
      `${Math.round(metrics.optimizationAcceptanceRate * 100)}%`,
      'Percentage of optimized time slots accepted',
    ],
    ['', '', ''], // Empty row separator
