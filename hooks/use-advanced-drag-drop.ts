...
export function useAdvancedDragDrop<T extends { id: string; startDate: Date; endDate: Date }>(
  ...
  const { snapToGrid = true, multiSelect = true, touchSupport = true, longPressDuration = 500 } = options
  ...
  const handleDragStart = useCallback(
    (itemId: string, event: React.MouseEvent | React.TouchEvent) => {
      ...
      setDragState((prev) => {
        const draggedItems = prev.selectedEvents.has(itemId) ? prev.selectedEvents : new Set([itemId])
  ...
  const handleDragEnd = useCallback(
    (event: MouseEvent | TouchEvent) => {
      ...
      if (dropTarget && dropTarget.dataset.date) {
        const targetDate = new Date(dropTarget.dataset.date)
        const daysDiff = Math.round((targetDate.getTime() - dragState.dragStartPosition.x) / (1000 * 60 * 60 * 24))
