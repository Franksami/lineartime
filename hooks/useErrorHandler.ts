...
export function useErrorHandler(options: UseErrorHandlerOptions = {}) {
  ...
  const captureError = useCallback(
    (error: Error | string, context?: Partial<ErrorInfo>) => {
  ...
  const tryAsync = useCallback(
    async <T>(fn: () => Promise<T>, context?: Partial<ErrorInfo>): Promise<T | null> => {
      try {
        return await fn();
      } catch (error) {
  ...
  const trySync = useCallback(
    <T>(fn: () => T, context?: Partial<ErrorInfo>): T | null => {
      try {
        return fn();
      } catch (error) {
        captureError(error as Error, {
  ...
  const withErrorBoundary = useCallback(
    <T extends (...args: any[]) => any>(fn: T, context?: Partial<ErrorInfo>): T => {
  ...
  const getError = useCallback(
    (errorId: string): ErrorInfo | undefined => {
  ...
  const retry = useCallback(
    async <T>(fn: () => Promise<T>, maxAttempts = 3, delay = 1000): Promise<T | null> => {
