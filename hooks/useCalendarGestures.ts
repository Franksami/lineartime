...
export function useCalendarGestures({
  ...
  const handleSwipe = useCallback(
    (movement: number[], velocity: number[]) => {
      const [mx] = movement;
      const [vx] = velocity;
  ...
  const handlePinch = useCallback(
    (scale: number, _origin: [number, number]) => {
  ...
  const handleLongPress = useCallback(
    (_point: [number, number], target: EventTarget) => {
  ...
  const handleDragSelection = useCallback(
    (_point: [number, number], target: EventTarget, isDragging: boolean) => {
      const element = target as HTMLElement;
      const dateStr = element.dataset.date;
      ...
      if (dateStr) {
        ...
        if (!isDragging && !selectionStart.current) {
          ...
          const _start = selectionStart.current < date ? selectionStart.current : date;
          const _end = selectionStart.current < date ? date : selectionStart.current;
