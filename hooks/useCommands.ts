...
export function useCommands() {
   ...
   */
  const executeCommand = useCallback(async (command: CommandDefinition, context?: any) => {
    try {
   ...
   */
  const searchCommands = useCallback((query: string): CommandDefinition[] => {
    ...
    if (!query.trim()) {
      ...
      const others = ALL_COMMANDS
        .filter(cmd => !prioritized.find(c => c.id === cmd.id))
