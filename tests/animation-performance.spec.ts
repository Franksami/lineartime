...
test.describe('Animation Performance Tests', () => {
  ...
  test('EventCard dropdown animation performance', async ({ page }) => {
    ...
    const performanceMetrics = await page.evaluate(() => {
      ...
      const fps = recentFrames.length > 1 
        ? 1000 / ((recentFrames[recentFrames.length - 1] - recentFrames[0]) / (recentFrames.length - 1))
        : 60;
    ...
    console.log(`EventCard dropdown animation: ${performanceMetrics.fps}fps, ${performanceMetrics.duration}ms`);
  });
  ...
  test('Modal animation performance', async ({ page }) => {
    ...
    const modalTrigger = page.locator('button').filter({ hasText: /create|add|new/i }).first();
    ...
    if (await modalTrigger.isVisible()) {
      ...
      console.log(`Modal open: ${modalMetrics.openDuration.toFixed(1)}ms, close: ${modalMetrics.closeDuration.toFixed(1)}ms`);
    }
  });
  ...
  test('Reduced motion accessibility compliance', async ({ page }) => {
    ...
    if (await viewSwitcher.isVisible()) {
      ...
      const switchTime = await page.evaluate(() => {
        performance.measure('reduced-motion-switch', 'reduced-motion-switch-start', 'reduced-motion-switch-end');
  ...
  test('Memory usage during animations', async ({ page }) => {
    ...
    if (baselineMemory > 0 && finalMemory > 0) {
      ...
      console.log(`Memory usage: baseline ${(baselineMemory / 1024 / 1024).toFixed(1)}MB, final ${(finalMemory / 1024 / 1024).toFixed(1)}MB (+${memoryIncreasePercent.toFixed(1)}%)`);
    }
  });
  ...
  test('AutoAnimate performance with large datasets', async ({ page }) => {
    ...
    const listElements = page.locator('[data-testid="event-list"], [data-testid="calendar-events"], ul li');
    ...
    if (elementCount > 10) {
      ...
      const filterButton = page.locator('button').filter({ hasText: /filter|search/i }).first();
      ...
      const largeDatasetMetrics = await page.evaluate(() => {
        ...
        return {
          duration: measure.duration,
          elementCount: document.querySelectorAll('[data-testid="event-list"] *, [data-testid="calendar-events"] *, ul li').length
        };
      });
      ...
      console.log(`Large dataset (${largeDatasetMetrics.elementCount} elements): ${largeDatasetMetrics.duration.toFixed(1)}ms`);
    }
  });
});
