...
test.describe('👤 Complete User Journey Testing - New, Existing & Premium Users', () => {
  ...
  test.describe('🆕 New User Journey', () => {
    test('should guide new user through complete onboarding flow', async ({ page }) => {
      ...
      const calendarContainer = page.locator('.linear-calendar-horizontal, [data-testid="linear-calendar"], .calendar-grid');
      const calendarVisible = await calendarContainer.first().isVisible({ timeout: 10000 }).catch(() => false);
      ...
      const signUpLink = page.locator('a[href*="sign-up"], button:has-text("Sign Up"), [data-testid="sign-up"]');
      const signUpExists = await signUpLink.first().isVisible().catch(() => false);
      ...
      if (signUpExists) {
        ...
        const hasSignUp = currentUrl.includes('/sign-up') || 
                         await page.locator('.cl-signUp, .clerk-sign-up, [data-clerk-element="signUp"]').isVisible();
      ...
      const appFunctionality = await page.evaluate(() => {
        ...
        const hasNavigation = document.querySelector('nav, .navigation, [role="navigation"]') !== null;
        const hasCalendar = document.querySelector('.calendar, .linear-calendar, [data-testid="calendar"]') !== null;
    ...
    test('should allow new user to create events without authentication', async ({ page }) => {
      ...
      await page.waitForSelector('.linear-calendar-horizontal, .calendar-grid, .day-cell', { timeout: 10000 });
      ...
      if (dayVisible) {
        ...
        const eventCreation = page.locator('.event-modal, .event-form, input[placeholder*="event"], [data-testid="event-create"]');
        const creationVisible = await eventCreation.first().isVisible().catch(() => false);
  ...
  test.describe('👤 Existing User Journey', () => {
    test('should handle returning user experience', async ({ page }) => {
      ...
      await page.addInitScript(() => {
        ...
        localStorage.setItem('lineartime_user', JSON.stringify({
          id: 'test-user-123',
          name: 'Test User',
          preferences: {
            theme: 'system',
        ...
        localStorage.setItem('lineartime_events', JSON.stringify([
          {
            id: '1',
      ...
      const hasTheme = await themeElements.first().isVisible().catch(() => false);
    ...
    test('should sync user data with Convex backend', async ({ page }) => {
      ...
      const realtimeIndicator = page.locator('.realtime-status, .connection-status, [data-connection-status]');
      const realtimeVisible = await realtimeIndicator.first().isVisible().catch(() => false);
  ...
  test.describe('💎 Premium User Journey', () => {
    test('should provide premium features and billing integration', async ({ page }) => {
      ...
      const billingLink = page.locator('a[href*="billing"], a[href*="subscription"], [data-testid="billing"]');
      const billingExists = await billingLink.first().isVisible().catch(() => false);
      ...
      if (billingExists) {
        ...
        const settingsMenu = page.locator('a[href*="settings"], [data-testid="settings"], button:has-text("Settings")');
        const settingsExists = await settingsMenu.first().isVisible().catch(() => false);
      ...
      const aiVisible = await aiAssistant.first().isVisible().catch(() => false);
  ...
  test.describe('🔄 Cross-User Journey Integration', () => {
    ...
    test('should maintain performance across all user types', async ({ page }) => {
      ...
      for (const userType of userTypes) {
        ...
        await page.goto('/');
        await page.waitForSelector('.linear-calendar-horizontal, .calendar-grid', { timeout: 15000 });
