...
test.describe('Event Creation in Improved Layout', () => {
  ...
  test('click-to-create event works with expanded layout', async ({ page }) => {
    ...
    if (modalCount > 0) {
      ...
      const inlineCreate = await page.locator('[class*="quick-create"], input[placeholder*="event"], input[placeholder*="title"]');
  ...
  test('drag-to-create multi-day event works', async ({ page }) => {
    ...
    if (startBox && endBox) {
      ...
      const eventUI = await page.locator('[role="dialog"], [class*="modal"], [class*="quick-create"], input[placeholder*="event"]');
  ...
  test('events display properly in expanded month rows', async ({ page }) => {
    ...
    const commandBar = await page.locator('[class*="command"], input[placeholder*="Type a command"]');
    if (await commandBar.count() > 0) {
      ...
      const eventElements = await page.locator('[class*="bg-green"], [class*="bg-blue"], [class*="event"]');
