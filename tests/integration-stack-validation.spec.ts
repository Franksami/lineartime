...
test.describe('🔗 Integration Stack Validation - Convex + Clerk + Stripe', () => {
  ...
  test.describe('🗄️ Convex Backend Integration', () => {
    test('should establish Convex connection and handle HTTP endpoints', async ({ page }) => {
      ...
      const webhookTest = await page.request.post('https://incredible-ibis-307.convex.cloud/clerk-user-webhook', {
    ...
    test('should validate Convex data operations', async ({ page }) => {
      ...
      const dataOperations = await page.evaluate(() => {
        ...
        const convexKeys = Object.keys(localStorage).filter(key => 
          key.includes('convex') || key.includes('clerk') || key.includes('auth')
        );
      ...
      const createEventButton = page.locator('button:has-text("Create"), button:has-text("Add"), .create-event, [data-testid="create-event"]');
      const createButtonVisible = await createEventButton.first().isVisible().catch(() => false);
      ...
      if (createButtonVisible) {
        ...
        const dataResult = await page.locator('.event-form, .modal, [role="dialog"], input[placeholder*="event"]').isVisible();
        if (dataResult) {
    ...
    test('should handle Clerk authentication flow', async ({ page }) => {
      ...
      const clerkConfig = await page.evaluate(() => {
        return {
          ...
          hasClerkElements: document.querySelectorAll('.cl-rootBox, [data-clerk-element]').length > 0
        };
      });
      ...
      const authElements = {
        signInButton: await page.locator('button:has-text("Sign In"), a[href*="sign-in"], .sign-in').isVisible().catch(() => false),
        signUpButton: await page.locator('button:has-text("Sign Up"), a[href*="sign-up"], .sign-up').isVisible().catch(() => false),
        userMenu: await page.locator('.user-menu, .profile-menu, [data-testid="user-menu"]').isVisible().catch(() => false)
      };
  ...
  test.describe('💳 Stripe Billing Integration', () => {
    ...
    test('should integrate billing with user interface', async ({ page }) => {
      ...
      const billingElements = {
        settingsLink: await page.locator('a[href*="settings"], button:has-text("Settings")').isVisible().catch(() => false),
        billingSection: await page.locator('.billing, [data-testid="billing"], .subscription').isVisible().catch(() => false),
        upgradeButton: await page.locator('button:has-text("Upgrade"), button:has-text("Subscribe")').isVisible().catch(() => false)
      };
      ...
      if (billingElements.settingsLink) {
        ...
        const billingInSettings = await page.locator('.billing-settings, [data-testid="billing"], .subscription-settings').isVisible().catch(() => false);
      ...
      if (billingElements.settingsLink || billingElements.upgradeButton || subscriptionStatus.hasPremium) {
  ...
  test.describe('🔄 Integration Flow Testing', () => {
    test('should handle complete user lifecycle integration', async ({ page }) => {
      ...
      const canCreateEvent = await page.evaluate(() => {
        ...
        const hasCreateButton = document.querySelector('button:has-text("Create"), .create-event, [data-testid="create"]');
