...
test.describe('📅 LinearCalendarHorizontal Integration - Complete System Validation', () => {
  ...
  test('should initialize LinearCalendarHorizontal component', async ({ page }) => {
    ...
    const calendarElement = await page
      .locator('.linear-calendar-horizontal, [data-testid="linear-calendar"], .calendar-grid')
      .first();
    await expect(calendarElement).toBeVisible();

    // Verify 12-month horizontal layout structure
    const monthElements = await page
      .locator('[data-month], .month-container, .calendar-month')
      .count();
    ...
    // Verify Command Center Calendar branding is preserved
    const pageContent = await page.content();
    const hasLinearTimeContent =
  ...
  test('should support drag and drop event creation', async ({ page }) => {
    ...
    const calendarDay = await page
      .locator('.day-cell, .calendar-day, [data-day], .grid-cell')
      .first();
    ...
    if (await calendarDay.isVisible()) {
      ...
      if (dayBoundingBox) {
        ...
        const eventForm = page.locator(
          'form, .event-form, input[placeholder*="event"], input[placeholder*="title"]'
        );

        // Give time for event creation UI to appear
        await page.waitForTimeout(1000);

        const modalVisible = await eventModal
          .first()
          .isVisible()
          .catch(() => false);
        const eventVisible = await newEvent
          .first()
          .isVisible()
          .catch(() => false);
        const formVisible = await eventForm
          .first()
          .isVisible()
          .catch(() => false);

        console.log(
          `📋 Event creation indicators: Modal=${modalVisible}, Event=${eventVisible}, Form=${formVisible}`
        );
  ...
  test('should handle event editing and interaction', async ({ page }) => {
    ...
    if (!(await existingEvent.isVisible())) {
      ...
      const eventCreated = await page
        ...
        .isVisible()
        .catch(() => false);
      const creationUI = await page
        .locator('.event-modal, input[placeholder*="event"]')
        .isVisible()
        .catch(() => false);
      ...
      if (creationUI) {
        ...
        const titleInput = await page
          .locator('input[placeholder*="event"], input[placeholder*="title"]')
          .first();
    ...
    if (await existingEvent.isVisible()) {
      ...
      const eventDetails = page.locator(
        '.event-modal, .event-popup, .event-details, [role="dialog"]'
      );
      const detailsVisible = await eventDetails
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test('should integrate with existing Command Center Calendar features', async ({ page }) => {
    console.log('🔗 Testing integration with Command Center Calendar features...');

    await page.waitForSelector('.fc', { timeout: 10000 });

    // Check for Command Center Calendar-specific features
    ...
    // Test navigation to other Command Center Calendar features
    const navElements = await page.locator(
      'nav, .navigation, a[href="/analytics"], a[href="/themes"]'
    );
    ...
    // Verify Command Center Calendar identity is preserved
    const pageContent = await page.content();
    const hasLinearTimeIdentity =
      pageContent.includes('Command Center Calendar') ||
      pageContent.includes('linear') ||
      pageContent.includes('horizontal') ||
      pageContent.includes('Life is bigger than a week');

    if (hasLinearTimeIdentity) {
      console.log('✅ Command Center Calendar brand identity preserved');
    } else {
      console.log('⚠️ Command Center Calendar branding may need attention');
    }

    console.log('✅ Command Center Calendar integration features validated');
  });
});
