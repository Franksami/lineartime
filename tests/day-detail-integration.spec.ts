...
test.describe('Day Detail View Integration', () => {
  ...
  test('should open day detail view when clicking on a day with events', async ({ page }) => {
    ...
    const todayCell = page.locator(`button:has-text("${currentDay}")`)
    ...
    await page.fill('textarea[placeholder="Description"]', 'This is a test event to verify stacking works')
  ...
  test('should toggle drag and resize modes in day detail view', async ({ page }) => {
    ...
    const dayWithEvents = page.locator('button').filter({ has: page.locator('div.absolute.bottom-0') }).first()
