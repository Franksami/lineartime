...
class AccessibilityTestUtils {
  ...
  async validateSemanticHTML() {
    const semanticStructure = await this.page.evaluate(() => {
      return {
        ...
        hasLandmarks: document.querySelectorAll('[role="banner"], [role="contentinfo"], [role="complementary"], [role="main"], [role="navigation"]').length,
  ...
  async testFocusManagement() {
    ...
    const modalTriggers = this.page.locator('[data-testid="modal-trigger"], button:has-text("Open"), [aria-haspopup="dialog"]')
...
test.describe('Accessibility Compliance - WCAG 2.1 AA Validation', () => {
  ...
  test.describe('Automated Accessibility Scanning', () => {
    test('should have no automatically detectable accessibility violations on main page', async ({ page }) => {
    ...
    test('should have no accessibility violations in playground components', async ({ page }) => {
      ...
      for (const playgroundPage of playgroundPages) {
        ...
        console.log(`${playgroundPage}: ${accessibilityScanResults.violations.length} total violations, ${criticalViolations.length} critical`)
      }
    })
    ...
    test('should validate modal and dialog accessibility', async ({ page }) => {
      ...
      for (const selector of dialogTriggers) {
        ...
        if (await trigger.count() > 0) {
          ...
          const accessibilityScanResults = await utils.scanWithAxe()
          const dialogViolations = accessibilityScanResults.violations.filter(v =>
            v.id.includes('dialog') || v.id.includes('modal') || v.id.includes('focus')
          )
  ...
  test.describe('Keyboard Navigation Testing', () => {
    ...
    test('should manage focus properly in interactive components', async ({ page }) => {
      ...
      const trigger = page.locator('[data-testid="modal-trigger"], button:has-text("Open")').first()
      if (await trigger.count() > 0) {
        await trigger.click()
        await page.waitForTimeout(300)
  ...
  test.describe('Screen Reader Support', () => {
    ...
    test('should have proper heading hierarchy', async ({ page }) => {
      ...
      console.log('Heading structure:', semanticStructure.headingStructure.map(h => `h${h.level}: ${h.text}`))
    })
  })
  ...
  test.describe('Cross-Browser Accessibility', () => {
    test('should maintain accessibility across different browsers', async ({ page, browserName }) => {
      const accessibilityScanResults = await utils.scanWithAxe()
      ...
      console.log(`${browserName}: ${accessibilityScanResults.violations.length} total violations, ${criticalViolations.length} critical`)
  ...
  test.describe('Mobile Accessibility', () => {
    ...
    test('should maintain accessibility on mobile devices', async ({ page }) => {
      ...
      const mobileViolations = accessibilityScanResults.violations.filter(v =>
        v.id.includes('touch') || v.id.includes('target') || v.impact === 'critical'
      )
