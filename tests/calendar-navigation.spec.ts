...
test.describe('Calendar Navigation', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="calendar-container"], .calendar, [class*="calendar"]', { 
      timeout: 10000 
    });
  });
  ...
  test('should switch between views', async ({ page }) => {
    ...
    const viewSwitcher = page.locator('[role="tablist"], [aria-label*="view"], button:has-text("Timeline")');
  ...
  test('should navigate between months on mobile', async ({ page, isMobile }) => {
    if (isMobile) {
      ...
      await page.waitForSelector('.mobile-calendar, [class*="MobileCalendar"]', { 
        timeout: 5000 
      }).catch(() => {});
      
      // Check for navigation buttons
      const prevButton = page.locator('button[aria-label*="prev" i], button:has(svg):left-of(:text("2024"))').first();
      const nextButton = page.locator('button[aria-label*="next" i], button:has(svg):right-of(:text("2024"))').first();
  ...
  test('should open AI Assistant', async ({ page }) => {
    ...
    const aiButton = page.locator('button:has(svg[class*="bot" i]), button[aria-label*="AI" i]').first();
    
    if (await aiButton.isVisible()) {
      await aiButton.click();
      
      // Wait for assistant panel to open
      await page.waitForSelector('.ai-assistant, [class*="AssistantPanel"]', { 
        timeout: 5000 
      }).catch(() => {});
      ...
      if (await assistantPanel.isVisible()) {
        // Try to close it
        const closeButton = page.locator('button[aria-label*="close" i], button:has(svg[class*="x" i])').first();
