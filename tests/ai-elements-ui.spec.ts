...
test.describe('🎨 AI Elements UI Components - Vercel AI SDK v5 Integration', () => {
  ...
  test('should open and display AI assistant panel', async ({ page }) => {
    ...
    const aiTrigger = page.locator('button:has-text("AI"), [data-testid="ai-assistant"], .ai-panel-trigger').first();
    ...
    if (await aiTrigger.count() > 0) {
      ...
      const conversation = page.locator('[role="log"], .conversation, [data-testid="conversation"]').first();
  ...
  test('should display prompt input with controls', async ({ page }) => {
    ...
    const promptInput = page.locator('textarea[placeholder*="message"], textarea[placeholder*="Ask"], [data-testid="prompt-input"]').first();
  ...
  test('should display model picker in prompt input', async ({ page }) => {
    ...
    const modelPicker = page.locator('[data-testid="model-select"], .model-picker, button:has-text("gpt")').first();
  ...
  test('should display web search toggle', async ({ page }) => {
    ...
    const webSearchToggle = page.locator('[data-testid="web-search-toggle"], button[title*="web search"], [aria-label*="web search"]').first();
  ...
  test('should display calendar context toggle', async ({ page }) => {
    ...
    const calendarToggle = page.locator('[data-testid="calendar-toggle"], button[title*="calendar"], [aria-label*="calendar context"]').first();
  ...
  test('should send message and display conversation', async ({ page }) => {
    ...
    // Send a message
    const promptInput = page.locator('textarea[placeholder*="message"], [data-testid="prompt-input"]').first();
    if (await promptInput.count() > 0) {
      ...
      const userMessage = page.locator('.message[data-role="user"], [data-testid="user-message"]').first();
      ...
      const aiMessage = page.locator('.message[data-role="assistant"], [data-testid="assistant-message"]').first();
  ...
  test('should display message actions (copy, regenerate, share, like, dislike)', async ({ page }) => {
    ...
    if (await aiTrigger.count() > 0) {
      ...
      if (await promptInput.count() > 0) {
        ...
        if (await reasoning.count() > 0) {
          ...
          const reasoningToggle = reasoning.locator('button, [data-testid="reasoning-toggle"]').first();
    ...
    await page.route('**/api/ai/chat', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          sources: [
            { id: 1, title: 'Calendar Events', url: '/calendar' }
          ]
        })
      });
    });
  ...
  test('should display tool execution (suggestSchedule with Apply button)', async ({ page }) => {
    ...
    if (await aiTrigger.count() > 0) {
      ...
      if (await promptInput.count() > 0) {
        ...
        const applyButton = page.locator('button:has-text("Apply"), [data-testid="apply-suggestion"]').first();
  ...
  test('should handle mobile responsive layout', async ({ page, browserName }) => {
    ...
    if (await aiTrigger.count() > 0) {
      ...
      const mobileHeader = page.locator('[data-testid="mobile-chat-header"], .mobile-header').first();
  ...
  test('should display loading states during AI responses', async ({ page }) => {
    ...
    if (await aiTrigger.count() > 0) {
      ...
      if (await promptInput.count() > 0) {
        ...
        const loadingIndicator = page.locator('[data-testid="loading"], .loading, .ai-thinking').first();
  ...
  test('should handle panel minimize/maximize', async ({ page }) => {
    ...
    if (await aiTrigger.count() > 0) {
      ...
      const minimizeButton = page.locator('[data-testid="minimize"], button[title*="minimize"], .minimize-button').first();
      if (await minimizeButton.count() > 0) {
        ...
        const maximizeButton = page.locator('[data-testid="maximize"], button[title*="maximize"], .maximize-button').first();
      ...
      const closeButton = page.locator('[data-testid="close"], button[title*="close"], .close-button').first();
