...
test.describe('Billing Integration E2E', () => {
  ...
  test.describe('Complete Billing Flow', () => {
    test('End-to-end billing flow: Sign-up → Settings → Pricing → Billing Management', async ({ page }) => {
      ...
      try {
        ...
        const settingsButton = page.locator('button:has-text("Settings"), [aria-label="Settings"], [data-testid="settings"]');
  ...
  test.describe('Error Handling', () => {
    test('Billing components handle unauthenticated state', async ({ page }) => {
      ...
      const hasGracefulHandling = content?.includes('sign in') ||
