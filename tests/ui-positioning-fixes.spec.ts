...
test.describe('UI Positioning Fixes', () => {
  ...
  test('Online badge should not overlap with Command Center Calendar header', async ({ page }) => {
    // Check that Command Center Calendar header is visible
  ...
  test('Filters button should not overlap with calendar content', async ({ page }) => {
    ...
    if (await janContent.isVisible()) {
      ...
      if (janBox && filtersBox) {
        ...
        const overlap = !(
          filtersBox.x > janBox.x + janBox.width ||
