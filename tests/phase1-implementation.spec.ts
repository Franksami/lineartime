...
async function createEventByDrag(
  page: Page,
  startX: number,
  startY: number,
  endX: number,
...
test.describe('📦 Phase 1: Component Decomposition', () => {
  ...
  test('CalendarGrid renders 12×42 grid structure', async ({ page }) => {
    ...
    const dayCells = page
      .locator('[role="gridcell"], [role="grid"] div')
  ...
  test('EventLayer displays and positions events correctly', async ({ page }) => {
    ...
    const eventElements = page.locator(
      '[class*="bg-green"], [class*="bg-blue"], [class*="bg-orange"], [class*="bg-purple"], [class*="event"]'
    );
...
test.describe('🎯 Phase 1: State Management', () => {
  ...
  test('CalendarContext provides state to components', async ({ page }) => {
    ...
    if (await firstDay.isVisible()) {
      ...
      const selectionIndicator = page.locator(
        '[class*="selected"], [class*="active"], [class*="primary"]'
      );
  ...
  test('Specialized hooks maintain separate concerns', async ({ page }) => {
    ...
    if ((await events.count()) > 0) {
      ...
      const toolbar = page
        .locator('[class*="toolbar"], [class*="floating"], [class*="absolute"]')
  ...
  test('Drag selection shows visual feedback', async ({ page }) => {
    ...
    if (gridBox) {
      ...
      const selection = page.locator(
        '[class*="blue-500"], [class*="selection"], [class*="border-2"]'
      );
  ...
  test('Quick title entry appears after drag', async ({ page }) => {
    ...
    if (gridBox) {
      ...
      const titleInput = page.locator(
        'input[placeholder*="title"], input[placeholder*="Title"], input[placeholder*="event"], input[type="text"]'
      );
...
test.describe('🛠 Phase 1: FloatingToolbar Enhancements', () => {
  ...
  test('FloatingToolbar appears on event click', async ({ page }) => {
    ...
    const events = page.locator(
      '[class*="bg-green"], [class*="bg-blue"], [class*="bg-orange"], [class*="event"]'
    );
  ...
  test('Conflict detection works', async ({ page }) => {
    ...
    if (gridBox) {
      ...
      if (await titleInput.isVisible({ timeout: 1000 }).catch(() => false)) {
        ...
        await expect(page.locator('text=Event 1'))
          .toBeVisible({ timeout: 1000 })
          .catch(() => {});
        await expect(page.locator('text=Event 2'))
          .toBeVisible({ timeout: 1000 })
          .catch(() => {});
      }
    }
