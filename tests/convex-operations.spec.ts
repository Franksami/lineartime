...
test.describe('Convex Operations - Database & Real-time', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const isAuthenticated = await page.locator('body').evaluate(() => {
      return !document.body.textContent?.includes('Sign in to LinearTime') &&
  ...
  test('Event CRUD Operations', async ({ page }) => {
    ...
    try {
      ...
      const createButton = page.locator('button:has-text("Create Event"), button:has-text("New Event"), button:has-text("Add Event"), [data-testid="create-event"]');
      const calendarCells = page.locator('[data-testid*="day"], [class*="day"], .calendar-cell, [role="button"]').first();
      ...
      // Fill event details
      const titleInput = page.locator('input[placeholder*="title"], input[placeholder*="Title"], input[name="title"], input[id="title"]');
      if (await titleInput.isVisible()) {
        await titleInput.fill('Test Event - Convex CRUD');
        
        // Look for description field
        const descInput = page.locator('textarea[placeholder*="description"], textarea[name="description"], input[name="description"]');
        ...
        const eventCreated = await page.locator('body').textContent().then(text => 
          text?.includes('Test Event - Convex CRUD') || false
        );
        ...
        if (await eventElement.isVisible()) {
          ...
          if (await editButton.isVisible()) {
            ...
            // Update the title
            const editTitleInput = page.locator('input[value*="Test Event"], input[placeholder*="title"]');
            if (await editTitleInput.isVisible()) {
              ...
              const eventUpdated = await page.locator('body').textContent().then(text =>
                text?.includes('Updated Test Event - Convex CRUD') || false
              );
        ...
        if (await updatedEvent.isVisible()) {
          ...
          if (await deleteButton.isVisible()) {
            ...
            const eventDeleted = await page.locator('body').textContent().then(text =>
              !text?.includes('Updated Test Event - Convex CRUD') || false
            );
  ...
  test('Data Isolation Between Users', async ({ page, context }) => {
    ...
    const userData = await page.evaluate(() => {
      ...
      return {
        hasUserData: document.body.textContent?.includes('Personal') ||
  ...
  test('Calendar and Category Operations', async ({ page }) => {
    ...
    if (hasCalendar) {
      ...
      const navButtons = page.locator('button:has-text("Previous"), button:has-text("Next"), [aria-label*="Previous"], [aria-label*="Next"]');
      ...
      const categoryButton = page.locator('button:has-text("Category"), button:has-text("Categories"), [data-testid*="category"]');
