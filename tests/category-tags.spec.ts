...
test.describe('🏷️ Category & Tag System Feature Testing', () => {
  ...
  test('should show available event categories', async ({ page }) => {
    ...
    await expect(page.locator('text=7 category types including Personal, Work, Effort')).toBeVisible()
    
    // Look for current selection display showing categories - use first() to avoid strict mode violation
    await expect(page.locator('text=Category:')).toBeVisible()
    await expect(page.locator('text=/personal|work|effort|note|meeting|deadline|milestone/').first()).toBeVisible()
  })
  ...
  test('should allow filtering by categories', async ({ page }) => {
    ...
    if (!filterFound) {
      const interactiveCategories = page.locator('button[class*="category"], .category-tag[role="button"], [data-category]')
  ...
  test('should show tag management interface', async ({ page }) => {
    ...
    if (!tagFound) {
      const eventCreation = page.locator('button:has-text("Create"), [data-testid="create-event"], .event-create')
  ...
  test('should display category statistics', async ({ page }) => {
    ...
    await expect(page.locator('text=/Categories|Category Distribution|Event Types/')).toBeVisible({ timeout: 10000 })
  })
