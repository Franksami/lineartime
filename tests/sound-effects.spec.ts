...
test.describe('Sound Effects Integration', () => {
  ...
  test('Sound settings are accessible and functional', async ({ page }) => {
    ...
    const settingsModal = page
      .locator('[role="dialog"], .settings, [data-testid*="settings"]')
      .first();
    ...
    const soundToggle = page
      .locator('#sound, [data-testid*="sound"], input[type="checkbox"][aria-label*="sound" i]')
      .first();
    if (await soundToggle.isVisible()) {
      ...
      if (!initialChecked) {
        // We just enabled sound
        const volumeControl = page
          .locator('#volume, input[type="range"][aria-label*="volume" i]')
          .first();
  ...
  test('Sound effects integrate with core operations', async ({ page }) => {
    ...
    const calendarExists = await page
      .locator('.calendar, [data-testid*="calendar"], .linear-calendar')
      .first()
      .isVisible();
    ...
    await page.evaluate(() => {
      ...
      try {
        ...
        audio
          ...
          .catch((error) => {
            (window as any)._autoplayTests.push({
              type: 'no-gesture',
              success: false,
              error: error.name,
            });
