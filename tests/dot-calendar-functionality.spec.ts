...
test.describe('Progress Calendar Dot View Functionality', () => {
  ...
  test('should be able to switch to dot view via settings', async ({ page }) => {
    ...
    await page.waitForSelector(
      '[role="dialog"], .settings-dialog, [data-testid="settings-dialog"]',
      { timeout: 5000 }
    );

    // Look for calendar day style setting
    const dayStyleSelect = page
      .locator('select', { hasText: 'Calendar Day Style' })
      .or(page.locator('[data-testid="calendar-day-style"]'))
      .or(page.getByLabel(/calendar.*day.*style/i))
      .first();
    ...
    if (await dayStyleSelect.isVisible()) {
      ...
      const dots = page.locator(
        '.rollingDigit, [style*="width: 5px"], [style*="width: var(--dot-size)"], .rounded-full'
      );
  ...
  test('should show days left counter in dot mode for current year', async ({ page }) => {
    ...
    if (await yearHeader.isVisible()) {
      ...
      const daysLeftContainer = page
        .locator('div.flex.items-baseline.gap-2:has-text("days left")')
        .first();
  ...
  test('should handle year navigation while preserving dot mode', async ({ page }) => {
    ...
    const yearControls = page.locator(
      'button:has-text("2024"), button:has-text("2023"), [data-testid*="year"]'
    );
