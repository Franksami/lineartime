...
test.describe('Toast UI Calendar Integration Tests', () => {
  ...
  test.describe('Calendar Loading and Initialization', () => {
    ...
    test('should load with proper CSS and styling', async ({ page }) => {
      ...
      await expect(calendarElement).toHaveCSS(
        'background-color',
    ...
    test('should create event by dragging on calendar grid', async ({ page }) => {
      ...
      if (timeSlotBox) {
        ...
        const isEventCreated =
          (await page.locator('.tui-full-calendar-weekday-schedule').count()) > 0;
      ...
      if (eventCount > 0) {
        ...
        if (eventBox) {
          ...
          const hasVisualFeedback =
            (await page.locator('.tui-full-calendar-weekday-schedule-title').count()) > 0;
      ...
      if (isPopupVisible) {
        ...
        if (await titleField.isVisible()) {
          ...
          if (await saveButton.isVisible()) {
            ...
            await expect(
              page.locator(
                '.tui-full-calendar-weekday-schedule-title:has-text("Test Event from Popup")'
              )
            ).toBeVisible();
          }
        }
  ...
  test.describe('Calendar Categories and Styling', () => {
    test('should display different calendar categories with colors', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(itemCount, 3); i++) {
        ...
        // Verify color is set
        const backgroundColor = await colorIndicator.evaluate(
    ...
    test('should toggle calendar visibility', async ({ page }) => {
      ...
      if (toggleCount > 0) {
        ...
        const visualFeedbackExists =
          (await page.locator('[data-testid="calendar-list"] .opacity-50').count()) > 0;
  ...
  test.describe('Accessibility Features', () => {
    test('should have proper ARIA labels and roles', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="nav-previous"]')).toHaveAttribute(
        'aria-label',
        /.*/
      );
    ...
    test('should have proper color contrast', async ({ page }) => {
      ...
      if (await calendarText.isVisible()) {
        const textColor = await calendarText.evaluate((el) => window.getComputedStyle(el).color);
        const backgroundColor = await calendarText.evaluate(
          (el) => window.getComputedStyle(el).backgroundColor
        );
