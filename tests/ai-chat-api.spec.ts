...
test.describe('🤖 AI Chat API (Vercel SDK v5) - API Endpoint Testing', () => {
  ...
  const mockStreamResponse = {
    choices: [
      {
        delta: {
          content: "I'll help you schedule that meeting. Let me check your calendar for available times."
        },
        finish_reason: null
      }
    ]
  ...
  test('POST /api/ai/chat - should handle explainConflicts tool', async ({ request }) => {
    ...
    const response = await request.post('/api/ai/chat', {
      data: {
        messages: [
          { role: 'user', content: 'Explain conflicts for January 15, 2025' }
        ],
        events: testEvents,
        userId: 'test-user',
        model: 'openai/gpt-4o-mini',
  ...
  test('POST /api/ai/chat - should handle listOpenSlots tool', async ({ request }) => {
    const response = await request.post('/api/ai/chat', {
      data: {
        messages: [
          { role: 'user', content: 'What are the open slots for January 15, 2025?' }
        ],
        events: [
          {
            id: '1',
  ...
  test('POST /api/ai/chat - should handle summarizePeriod tool', async ({ request }) => {
    ...
    const response = await request.post('/api/ai/chat', {
      data: {
        messages: [
          { role: 'user', content: 'Summarize my calendar from January 10-20, 2025' }
        ],
        events: testEvents,
        userId: 'test-user',
        model: 'openai/gpt-4o-mini',
  ...
  test('POST /api/ai/chat - should include/exclude calendar events based on flag', async ({ request }) => {
    const testEvents = [
      {
        id: '1',
  ...
  test('POST /api/ai/chat - should handle web search flag', async ({ request }) => {
    const response = await request.post('/api/ai/chat', {
      data: {
        messages: [
          { role: 'user', content: 'What are current best practices for team meetings?' }
        ],
        events: [],
        userId: 'test-user',
        model: 'openai/gpt-4o-mini',
  ...
  test('POST /api/ai/chat - should handle Convex persistence errors gracefully', async ({ request }) => {
  ...
  test('POST /api/ai/chat - should include system prompt with current date', async ({ request }) => {
    const response = await request.post('/api/ai/chat', {
      data: {
  ...
  test('POST /api/ai/chat - should handle tool execution errors', async ({ request, page }) => {
    ...
    const response = await request.post('/api/ai/chat', {
      data: {
        messages: [
          { role: 'user', content: 'This should trigger a tool error' }
        ],
        events: [],
        userId: 'test-user',
        model: 'openai/gpt-4o-mini',
