...
test.describe('Performance Monitoring Integration', () => {
  ...
  test('verify SLO monitoring can be started and stopped', async ({ page }) => {
    ...
    if (!isRunning) {
      ...
      const realtimeIndicator = await page
        .locator('text="Real-time monitoring active"')
        .isVisible();
  ...
  test('verify performance dashboard tabs functionality', async ({ page }) => {
    ...
    const tabs = ['slo-dashboard', 'core-web-vitals', 'design-system', 'trend-analysis'];
  ...
  test('verify SLO compliance calculation', async ({ page }) => {
    ...
    const complianceText = await page
      .locator('div:has-text("Overall SLO Compliance") + div .text-3xl')
      .textContent();
    ...
    if (complianceText) {
      ...
      console.log(
        `SLO Compliance: ${percentage}% (${percentage >= 95 ? 'PASS' : 'NEEDS IMPROVEMENT'})`
      );
    }
  });
