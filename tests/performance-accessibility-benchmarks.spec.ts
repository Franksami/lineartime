...
test.describe('⚡ Performance & Accessibility Benchmarks', () => {
  ...
  test.describe('⚡ Performance Benchmarks', () => {
    test('should measure core performance metrics', async ({ page }) => {
      ...
      const appFunctional = await page.evaluate(() => {
        const hasCalendar = document.querySelector(
          '.linear-calendar-horizontal, .calendar-grid, .calendar'
        );
    ...
    test('should measure calendar-specific performance', async ({ page }) => {
      ...
      const calendarPresent = await page
        .waitForSelector('.linear-calendar-horizontal, .calendar-grid, .calendar, body', {
          timeout: 15000,
        })
        .catch(() => null);
  ...
  test.describe('♿ Accessibility Benchmarks', () => {
    ...
    test('should test keyboard navigation functionality', async ({ page }) => {
      ...
      const hasSkipLinks = await skipLinks
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe('📊 Comprehensive Performance Report', () => {
    test('should generate performance and accessibility summary', async ({ page }) => {
      ...
      const performanceScore = {
        ...
        domReady:
          domReadyTime < 3000 ? 100 : Math.max(0, 100 - Math.floor((domReadyTime - 3000) / 50)),
        resourceCount:
          comprehensiveMetrics.performanceTiming.resourceCount < 50
            ? 100
            : Math.max(0, 100 - (comprehensiveMetrics.performanceTiming.resourceCount - 50)),
      };

      const avgPerformanceScore = Math.round(
        (performanceScore.loadTime + performanceScore.domReady + performanceScore.resourceCount) / 3
      );

      // Generate accessibility score
      const accessibilityScore = {
        structure:
          comprehensiveMetrics.accessibilityAnalysis.hasMainElement &&
          comprehensiveMetrics.accessibilityAnalysis.headingCount > 0
            ? 100
            : 50,
        ariaSupport:
          comprehensiveMetrics.accessibilityAnalysis.ariaLabels > 5
            ? 100
            : comprehensiveMetrics.accessibilityAnalysis.ariaLabels > 0
              ? 75
              : 25,
        images:
          comprehensiveMetrics.pageAnalysis.images > 0
            ? (comprehensiveMetrics.accessibilityAnalysis.altTextImages /
                comprehensiveMetrics.pageAnalysis.images) *
              100
            : 100,
        navigation:
          comprehensiveMetrics.accessibilityAnalysis.focusableElements > 10
            ? 100
            : comprehensiveMetrics.accessibilityAnalysis.focusableElements > 5
              ? 75
              : 50,
      };

      const avgAccessibilityScore = Math.round(
        (accessibilityScore.structure +
          accessibilityScore.ariaSupport +
          accessibilityScore.images +
          accessibilityScore.navigation) /
          4
      );
      ...
      const finalReport = {
        ...
        performance: {
          ...
          memoryUsage: comprehensiveMetrics.memoryUsage
            ? `${comprehensiveMetrics.memoryUsage.used}MB`
            : 'N/A',
        },
        accessibility: {
