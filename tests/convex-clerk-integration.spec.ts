...
test.describe('Convex + Clerk + Stripe Integration Suite', () => {
  test.describe('Authentication & User Management', () => {
    ...
    test('User deletion webhook cleanup', async ({ page }) => {
      ...
      console.log('  - Cascades deletion of: events, categories, calendars, providers, sync queue, AI sessions');
      console.log('  - Uses batch processing for performance');
    });
  });

  test.describe('Billing System Integration', () => {
    test('Billing API endpoints handle missing Stripe keys gracefully', async ({ page, request }) => {
    ...
    test('Free tier subscription initialization', async ({ page }) => {
      ...
      if (await billingTab.isVisible()) {
        ...
        const hasFreeIndicators = billingContent?.includes('Free') || 
        ...
        if (hasFreeIndicators) {
          ...
          const hasUpgradeOptions = billingContent?.includes('Upgrade') || 
        ...
        console.log('ℹ️ Billing tab not found - may require authentication or settings structure update');
      }
    });
    ...
    test('Subscription status without authentication', async ({ page }) => {
      ...
      const hasPricingInfo = pricingContent?.includes('Free') || 
  ...
  test.describe('Convex Integration', () => {
    test('Convex HTTP endpoints are accessible', async ({ page, request }) => {
      ...
      const convexUrl = process.env.NEXT_PUBLIC_CONVEX_URL || 'https://incredible-ibis-307.convex.cloud';
    ...
    test('Event operations with authentication state', async ({ page }) => {
      ...
      if (hasCalendar) {
        ...
        const createButton = page.locator('[data-testid="create-event"], button:has-text("Create")');
  ...
  test.describe('Error Handling & Resilience', () => {
    ...
    test('Pages handle missing environment variables gracefully', async ({ page }) => {
      ...
      for (const pagePath of testPages) {
        ...
        const hasErrors = content?.includes('Something went wrong') || 
  ...
  test.describe('Data Flow Integration', () => {
    test('User preferences sync between Clerk and Convex', async ({ page }) => {
      ...
      const settingsContent = await page.textContent('body');
      const hasSettingsOptions = settingsContent?.includes('Theme') || 
      ...
      if (hasSettingsOptions) {
        ...
        const themeToggle = page.locator('button:has-text("Dark"), button:has-text("Light"), [data-testid*="theme"]');
        ...
        const settingsPersisted = reloadedContent?.includes('Settings') || 
                                 reloadedContent?.includes('Theme');
    ...
    test('Event data consistency across authentication state changes', async ({ page }) => {
      ...
      const calendarContent = await page.textContent('body');
      const hasEventData = calendarContent?.includes('Event') || 
                          calendarContent?.includes('Calendar') ||
      ...
      if (hasEventData) {
        ...
        const dataConsistent = consistentContent?.includes('Calendar') || 
                              consistentContent?.length > 500;
  ...
  test.describe('Security & Compliance', () => {
    ...
    test('Authentication state security', async ({ page }) => {
      ...
      const hasAuthPrompt = settingsContent?.includes('Sign in') || 
