...
test.describe('🗄️ Convex AI Chat Persistence - Database Integration Tests', () => {
  ...
  const testMessages = [
    ...
    },
    {
      role: 'assistant', 
      parts: [
        { type: 'text', text: 'I\'d be happy to help you schedule a meeting. Let me check your available times.' },
        { 
          type: 'tool-call',
  ...
  test('should persist and retrieve messages via appendMessages', async ({ page }) => {
    ...
    // Add messages
    await page.evaluate(async ({ chatId, messages }) => {
      // @ts-ignore
      const convex = window.convex;
  ...
  test('should log AI events for analytics via logEvent', async ({ page }) => {
    ...
    const eventTypes = [
      ...
      { type: 'applySuggestion', data: { suggestionId: 'sug_456', startISO: '2025-01-15T14:00:00.000Z' } },
      { type: 'share', data: { chatId, method: 'link' } }
    ];
    
    // Log each event type
    for (const event of eventTypes) {
      await page.evaluate(async ({ userId, chatId, eventType, data }) => {
        // @ts-ignore
        const convex = window.convex;
  ...
  test('should manage multiple chats via createChat and listChats', async ({ page }) => {
    ...
    for (const title of chatTitles) {
      const chatId = await page.evaluate(async ({ userId, title }) => {
        // @ts-ignore
        const convex = window.convex;
  ...
  test('should handle concurrent chat operations', async ({ page }) => {
    ...
    const concurrentChatPromises = Array.from({ length: 5 }, (_, i) => 
      page.evaluate(async ({ userId, title }) => {
        // @ts-ignore
        const convex = window.convex;
        return await convex.mutation('aiChat:createChat', { userId, title });
    ...
    const messagePromises = chatIds.map(chatId => 
      page.evaluate(async ({ chatId, messages }) => {
        // @ts-ignore
        const convex = window.convex;
        await convex.mutation('aiChat:appendMessages', { chatId, messages });
      }, { chatId, messages: testMessages })
  ...
  test('should handle error cases gracefully', async ({ page }) => {
    ...
    const malformedMessageAttempt = await page.evaluate(async ({ chatId }) => {
      try {
        // @ts-ignore
        const convex = window.convex;
        await convex.mutation('aiChat:appendMessages', {
          chatId: chatId,
          messages: [
            { role: 'invalid_role', parts: 'not_an_array' }
          ]
        });
        return { success: true };
      } catch (error) {
  ...
  test('visual regression - persistence layer performance', async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      operations.push(
        page.evaluate(async ({ userId, index }) => {
          // @ts-ignore
          const convex = window.convex;
          ...
          await convex.mutation('aiChat:appendMessages', {
            chatId,
            messages: [
              { role: 'user', parts: [{ type: 'text', text: `Performance test message ${index}` }] },
              { role: 'assistant', parts: [{ type: 'text', text: `Performance response ${index}` }] }
            ]
          });
    ...
    console.log(`Persistence Performance - Operations: ${operationTime}ms, Retrieval: ${retrievalTime}ms`);
  });
