        ...
        if (hasCalendar) {
          ...
          try {
            ...
            const createButton = page.locator(
              'button:has-text("Create"), button:has-text("Add Event"), [data-testid="create-event"]'
            );
            const calendarCells = page
              .locator('[data-testid*="day"], [class*="day"], .calendar-cell')
              .first();
            ...
            const eventForm = page.locator(
              'form:has(input[placeholder*="title"]), [data-testid="event-form"], input[placeholder*="Event"]'
            );
    ...
    test('All major features integrate properly', async ({ page }) => {
      ...
      for (const feature of featureTests) {
        try {
          ...
          console.log(
            `${result ? '✅' : '⚠️'} ${feature.name}: ${result ? 'Working' : 'Needs attention'}`
          );
        } catch (error) {
  ...
  test.describe('Performance Integration', () => {
    test('Complete system performs within acceptable limits', async ({ page }) => {
      ...
      for (const test of performanceTests) {
        ...
        console.log(
          `${withinThreshold ? '✅' : '⚠️'} ${test.name}: ${loadTime}ms (threshold: ${test.threshold}ms)`
        );
    ...
    test('System handles various error conditions gracefully', async ({ page }) => {
      ...
      for (const scenario of errorScenarios) {
        try {
          ...
          console.log(
            `${recovered ? '✅' : '❌'} ${scenario.name}: ${recovered ? 'Handled gracefully' : 'Needs improvement'}`
          );
        } catch (error) {
    ...
    test('Complete system works on mobile devices', async ({ page }) => {
      ...
      for (const test of mobileTests) {
        ...
        const mobileMenu = page.locator(
          'button[aria-label*="menu"], button:has-text("☰"), .mobile-menu-toggle'
        );
