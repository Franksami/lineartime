...
test.describe('🔍 Comprehensive UI Testing', () => {
  ...
  test('Foundation Structure Check', async ({ page }) => {
    ...
    // Check month labels
    const monthLabels = await page.locator('text=/Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec/').all()
  ...
  test('Accessibility Quick Check', async ({ page }) => {
    ...
    if (await firstButton.count() > 0) {
      ...
      console.log(`Focus styles: outline="${focusStyles.outline}", boxShadow="${focusStyles.boxShadow}"`)
    }
  })
  ...
  test('Data Validation Check', async ({ page }) => {
    ...
    const hasCalendarData = await page.evaluate(() => {
      return window.localStorage.getItem('calendar-events') !== null ||
