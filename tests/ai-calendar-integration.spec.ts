...
test.describe('📅 AI-Calendar Integration - End-to-End Calendar Integration Tests', () => {
  ...
  test('should integrate AI scheduling suggestions with calendar events', async ({ page }) => {
    ...
    const calendarEvent = page.locator('[data-testid="calendar-event"], .event-card').filter({ hasText: 'Team Meeting' });
  ...
  test('should find and suggest available time slots', async ({ page }) => {
    ...
    await page.route('**/api/ai/chat', async route => {
      if (route.request().method() === 'POST') {
        ...
        if (postData.messages.some((m: any) => m.content.includes('available') || m.content.includes('free'))) {
          const mockResponse = {
            type: 'tool-call',
  ...
  test('should provide calendar period summaries through AI', async ({ page }) => {
    ...
    await page.route('**/api/ai/chat', async route => {
      if (route.request().method() === 'POST') {
        ...
        if (postData.messages.some((m: any) => m.content.includes('summarize') || m.content.includes('summary'))) {
          const mockResponse = {
            type: 'tool-call',
  ...
  test('should respect calendar context settings (include/exclude)', async ({ page }) => {
    ...
    const calendarToggle = page.locator('input[type="checkbox"]').filter({ hasText: /calendar|context/i });
  ...
  test('should handle complex scheduling scenarios with multiple constraints', async ({ page }) => {
    ...
    const aiInput = page.locator('input[placeholder*="Ask"], textarea[placeholder*="message"]').first();
    await aiInput.fill('Schedule a 90-minute weekly team planning meeting for next week. Find the best time considering everyone\'s availability and productivity patterns.');
    await page.keyboard.press('Enter');
