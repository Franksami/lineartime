...
test.describe('Mobile Responsiveness - Comprehensive Testing', () => {
  ...
  test.describe('Touch Interactions', () => {
    ...
    test('should handle pinch-to-zoom appropriately', async ({ page }) => {
      ...
      const hasZoomControl = viewportMeta.includes('user-scalable') || viewportMeta.includes('maximum-scale')
  ...
  test.describe('Mobile Navigation Patterns', () => {
    test('should implement mobile navigation correctly', async ({ page }) => {
      ...
      if (navigation.found) {
        ...
        const hasVisibleMenu = await page.locator('[role="menu"], .navigation-menu, [data-testid="nav-menu"]').isVisible()
    ...
    test('should support bottom navigation on mobile', async ({ page }) => {
      ...
      const bottomNav = page.locator('[data-testid="bottom-nav"], .bottom-navigation, nav[role="tablist"]')
