...
test.describe('Phase 6.1.1 Market Validation Research Infrastructure', () => {
  ...
  test('Interactive Research Analytics Tab System works', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: /timeline analytics/i })).toHaveAttribute(
      'data-state',
      'active'
    );
    ...
    await expect(page.getByRole('tab', { name: /a\/b testing/i })).toHaveAttribute(
      'data-state',
      'active'
    );
  ...
  test('HorizontalTimelineAnalytics component renders', async ({ page }) => {
    ...
    await expect(
      page.getByText('Validating "Life is bigger than a week" hypothesis')
    ).toBeVisible();
  ...
  test('Strategic Validation Summary displays correctly', async ({ page }) => {
    ...
    await expect(
      page.getByText('✅ VALIDATION OUTCOME: Proceed with Horizontal Timeline')
    ).toBeVisible();
  ...
  test('A/B Testing Framework tab navigation works', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: /trends/i })).toHaveAttribute(
      'data-state',
      'active'
    );
  });
