...
test.describe('⚡ AI Performance Testing - Load, Memory, and Responsiveness Tests', () => {
  ...
  test('should meet AI API response time benchmarks', async ({ page }) => {
    ...
    await page.waitForSelector('text=This is a test response', { timeout: PERFORMANCE_BENCHMARKS.apiResponseTime });
  ...
  test('should handle streaming responses efficiently', async ({ page }) => {
    ...
    await page.waitForSelector('text*=time slots', { timeout: PERFORMANCE_BENCHMARKS.streamingResponseTime });
  ...
  test('should handle tool execution within performance limits', async ({ page }) => {
    ...
    await page.waitForSelector('text=2:00 PM', { timeout: PERFORMANCE_BENCHMARKS.toolExecutionTime });
  ...
  test('should handle concurrent AI requests without significant performance degradation', async ({ page }) => {
    ...
    console.log(`Concurrent Requests Time: ${concurrentTime.toFixed(2)}ms for ${PERFORMANCE_BENCHMARKS.concurrentRequests} requests`);
  });
  ...
  test('should properly clean up resources after AI operations', async ({ page }) => {
    ...
    const closeButton = page.locator('button[aria-label="Close"], button:has-text("Close"), [data-testid="close"]');
