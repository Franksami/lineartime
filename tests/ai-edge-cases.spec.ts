...
test.describe('🔒 AI Edge Cases & Security - Robustness and Security Testing', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Open AI Assistant
    await page.click('button[aria-label="Open AI Assistant"], button:has-text("AI"), [data-testid="ai-assistant"]');
  ...
  test('should sanitize and prevent XSS in AI responses', async ({ page }) => {
    ...
    const aiInput = page.locator('input[placeholder*="Ask"], textarea[placeholder*="message"]').first();
    ...
    const responseText = await page.locator('[data-testid="ai-message"], .ai-response, .message').last().textContent();
    if (responseText) {
  ...
  test('should prevent calendar data leakage between users', async ({ page }) => {
    ...
    for (const attempt of crossUserAttempts) {
      ...
      const response = await page.locator('[data-testid="ai-message"], .ai-response, .message').last().textContent();
      if (response) {
  ...
  test('should prevent resource exhaustion attacks', async ({ page }) => {
    ...
    await page.route('**/api/ai/chat', async route => {
      ...
      if (inputSize > 10000) { // 10KB limit example
        await route.fulfill({
          status: 413,
          contentType: 'text/plain',
  ...
  test('should handle concurrent access edge cases', async ({ page }) => {
    ...
    const responses = await page.locator('[data-testid="ai-message"], .ai-response, .message').count();
  ...
  test('should sanitize tool execution parameters', async ({ page }) => {
    ...
    await page.route('**/api/ai/chat', async route => {
      ...
      const hasMaliciousParams = JSON.stringify(postData).includes('<script>') ||
  ...
  test('should prevent information disclosure in error messages', async ({ page }) => {
    ...
    const sensitiveErrors = [
      { status: 500, body: JSON.stringify({ error: 'Database connection failed: password=secret123' }) },
      ...
      { status: 502, body: JSON.stringify({ error: 'Upstream server error: http://internal-api:8080' }) }
    ];
    ...
    for (const [index, errorResponse] of sensitiveErrors.entries()) {
      ...
      if (displayedText) {
        ...
        const hasGenericError = displayedText.includes('something went wrong') ||
  ...
  test('comprehensive security validation - penetration testing simulation', async ({ page }) => {
    ...
    await page.route('**/api/ai/chat', async route => {
      if (Math.random() > 0.7) { // Simulate rate limiting
        rateLimitHit = true;
