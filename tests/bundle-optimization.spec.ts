...
class BundleTestUtils {
  ...
  async checkTreeShaking() {
    ...
    const unusedFrameworks = [
      'chakra', 'mantine', 'antd', '@mui/material', '@emotion/react'
    ]
    
    const foundUnusedFrameworks = unusedFrameworks.filter(framework =>
      loadedAssets.some(asset => 
        asset.toLowerCase().includes(framework.toLowerCase())
      )
    )
...
test.describe('Bundle Optimization - Framework Migration Validation', () => {
  ...
  test.describe('Framework Removal Verification', () => {
    test('should not load Chakra UI assets', async ({ page }) => {
      ...
      const chakraAssets = loadedAssets.filter(asset => 
        asset.toLowerCase().includes('chakra') ||
        asset.includes('@chakra-ui')
      )
    ...
    test('should not load Mantine assets', async ({ page }) => {
      ...
      const mantineAssets = loadedAssets.filter(asset =>
        asset.toLowerCase().includes('mantine') ||
        asset.includes('@mantine/')
      )
    ...
    test('should not load Ant Design assets', async ({ page }) => {
      ...
      const antdAssets = loadedAssets.filter(asset =>
        asset.toLowerCase().includes('antd') ||
        asset.includes('ant-design')
      )
    ...
    test('should not load MUI (Material-UI) assets', async ({ page }) => {
      ...
      const muiAssets = loadedAssets.filter(asset =>
        asset.includes('@mui/') ||
  ...
  test.describe('shadcn/ui Component Validation', () => {
    test('should use shadcn/ui components correctly', async ({ page }) => {
      ...
      if (await card.count() > 0) {
        const cardInfo = await utils.verifyComponentFramework('[data-testid*="card"], .card', 'shadcn')
    ...
    test('should have working theme system', async ({ page }) => {
      ...
      const themeToggle = page.locator('[data-testid="theme-toggle"], [aria-label*="theme"], button:has-text("Dark"), button:has-text("Light")')
  ...
  test.describe('Code Splitting Validation', () => {
    ...
    test('should lazy load playground components', async ({ page }) => {
      ...
      const playgroundSections = ['/playground/shell', '/playground/command', '/playground/contexts']
  ...
  test.describe('Cross-Browser Optimization Validation', () => {
    test('should work consistently across browsers after optimization', async ({ page, browserName }) => {
      await page.goto('/')
      await page.waitForLoadState('networkidle')
  ...
  test.describe('Error Handling After Optimization', () => {
    test('should handle missing framework dependencies gracefully', async ({ page }) => {
      ...
      const frameworkErrors = consoleErrors.filter(error =>
