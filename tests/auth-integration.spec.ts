...
test.describe('Auth Integration - Convex + Clerk', () => {
  ...
  test('Sign-in with existing user', async ({ page }) => {
    ...
    try {
      ...
      const errorMessage = await page.locator('text=Invalid').textContent().catch(() => '');
      if (errorMessage) {
  ...
  test('User data persistence in Convex after sign-up', async ({ page }) => {
    ...
    const isAuthenticated = await page.locator('body').evaluate(() => {
      ...
      return document.querySelector('[data-testid="user-button"]') !== null ||
  ...
  test('Protected routes require authentication', async ({ page }) => {
    ...
    for (const route of protectedRoutes) {
      ...
      console.log(`Route ${route}: ${isOnSignIn ? 'Redirected to sign-in' : 'Showing protected content'}`);
  ...
  test('User sign-out flow', async ({ page }) => {
    ...
    const userButton = page.locator('[data-testid="user-button"], [aria-label="User menu"], button:has-text("Profile")');
