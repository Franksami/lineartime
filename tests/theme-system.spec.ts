...
test.describe('🎨 Advanced Theme System Feature Testing', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="linear-calendar"], .linear-calendar, body', {
      timeout: 10000,
    });
  });
  ...
  test('should display theme selector', async ({ page }) => {
    ...
    if (!themeUIFound) {
      ...
      const hasThemeClass = themeClasses.some(
        (themeClass) => bodyClasses?.includes(themeClass) || htmlClasses?.includes(themeClass)
      );
  ...
  test('should persist theme preferences', async ({ page }) => {
    ...
    const themeToggle = page
      .locator(
        'button:has-text("Dark"), [data-testid="theme-toggle"], [data-testid="dark-mode-toggle"]'
      )
      .first();
  ...
  test('should support high contrast themes', async ({ page }) => {
    ...
    const calendarElement = page
      .locator('.calendar-grid, [data-testid="linear-calendar"], .linear-calendar')
      .first();
  ...
  test('should handle theme transitions smoothly', async ({ page }) => {
    const themeToggle = page
      .locator('button:has-text("Dark"), button:has-text("Light"), [data-testid="theme-toggle"]')
      .first();
  ...
  test('should work with calendar event colors', async ({ page }) => {
    ...
    if ((await eventElements.count()) > 0) {
      ...
      const hasThemedColors =
        eventStyles.backgroundColor !== 'rgba(0, 0, 0, 0)' ||
  ...
  test('should be responsive across different devices', async ({ page }) => {
    ...
    const themeControl = page
      .locator('button:has-text("Theme"), button:has-text("Dark"), [data-testid="theme-toggle"]')
      .first();
