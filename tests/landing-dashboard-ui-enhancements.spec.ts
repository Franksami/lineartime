...
test.describe('Landing Page & Dashboard UI/UX Enhancements', () => {
  test.describe('Landing Page (/landing)', () => {
    ...
    test('should render hero section with key elements', async ({ page }) => {
      ...
      await expect(page.locator('h1:has-text("LinearTime Calendar")')).toBeVisible();
      ...
      // Check CTA buttons
      await expect(page.locator('button:has-text("Get Started")').or(
        page.locator('a:has-text("Get Started")')
      )).toBeVisible();
      
      await expect(page.locator('button:has-text("Learn More")').or(
    ...
    test('should have responsive navigation with Clerk authentication', async ({ page }) => {
      ...
      await expect(page.locator('header span:has-text("LinearTime")').first()).toBeVisible();
      await expect(page.locator('text=Beta').or(page.locator('[data-testid="beta-badge"]')).first()).toBeVisible();
    ...
    test('should display features section with grid layout', async ({ page }) => {
      ...
      await expect(page.locator('h2:has-text("Features")').or(
      ...
      const featureCards = page.locator('[data-testid="feature-card"]').or(
        page.locator('.grid').locator('.border').or(
          page.locator('h3')
        )
      );
      
      // Should have multiple feature cards
      await expect(featureCards).toHaveCount(3, { timeout: 5000 }).catch(async () => {
    ...
    test('should have testimonials section', async ({ page }) => {
      ...
      await expect(page.locator('text=/Testimonials/').or(
        page.locator('[data-testid="testimonials-section"]')
      )).toBeVisible();
      
      // Check for testimonial content
      const testimonialContent = page.locator('[data-testid="testimonial"]').or(
        page.locator('.testimonial').or(
    ...
    test('should be mobile responsive', async ({ page }) => {
      ...
      await expect(page.locator('text=LinearTime')).toBeVisible();
  ...
  test.describe('Enhanced Dashboard Overview', () => {
    test.beforeEach(async ({ page }) => {
      ...
      const dashboardExists = await page.locator('text="Dashboard Overview"').or(
        page.locator('text="Total Events"')
      ).isVisible();
      
      if (!dashboardExists) {
        // Try navigating to manage view or other potential dashboard locations
        const manageButton = page.locator('text="Manage"').or(page.locator('[data-testid="manage-view"]'));
    ...
    test('should display dashboard overview with key metrics or skip if not available', async ({ page }) => {
      // Look for dashboard overview component
      const dashboardOverview = page.locator('[data-testid="dashboard-overview"]').or(
        page.locator('text="Dashboard Overview"').or(
          page.locator('text="Total Events"')
        )
      );
      ...
      if (await dashboardOverview.first().isVisible({ timeout: 2000 })) {
        ...
        await expect(page.locator('text="Total Events"').or(
          page.locator('[data-testid="total-events"]')
        ).first()).toBeVisible();
        
        await expect(page.locator('text="Upcoming Today"').or(
          page.locator('text="Upcoming Events"')
        ).first()).toBeVisible();
        
        await expect(page.locator('text="Completed Today"').or(
          page.locator('text="Focus Time"')
        ).first()).toBeVisible();
      } else {
    ...
    test('should show upcoming events section or skip if not available', async ({ page }) => {
      const upcomingSection = page.locator('text="Upcoming Events"').or(
      ...
      if (await upcomingSection.first().isVisible({ timeout: 2000 })) {
        ...
        const eventCards = page.locator('[data-testid="event-card"]').or(
          page.locator('.border').locator('h3')
        );
        
        const emptyState = page.locator('text="No upcoming events"').or(
          page.locator('text="Your calendar is clear"')
        );
    ...
    test('should display activity feed with proper icons', async ({ page }) => {
      ...
      if (await activitySection.isVisible()) {
        ...
        const activityItems = page.locator('[data-testid="activity-item"]').or(
          page.locator('text="Event completed"').or(
    ...
    test('should show calendar sync status', async ({ page }) => {
      const syncSection = page.locator('text="Calendar Sync"').or(
        page.locator('[data-testid="calendar-sync"]')
      );
      
      if (await syncSection.isVisible()) {
        // Should show sync status or setup message
        const syncStatus = page.locator('text="Synced"').or(
          page.locator('text="No calendar providers"').or(
            page.locator('text="Connect Calendars"')
          )
        );
    ...
    test('should have functional privacy toggle for metrics', async ({ page }) => {
      const privacyToggle = page.locator('[data-testid="privacy-toggle"]').or(
        page.locator('button').filter({ hasText: /eye/i }).first()
      );
    ...
    test('should handle loading states gracefully', async ({ page }) => {
      ...
      const loadingIndicators = page.locator('text="Loading"').or(
    ...
    test('should display productivity insights with trends', async ({ page }) => {
      const insightsSection = page.locator('text="Productivity Insights"').or(
        page.locator('[data-testid="productivity-insights"]')
      );
      
      if (await insightsSection.isVisible()) {
        // Should show trend indicators
        const trendIndicators = page.locator('svg').filter({ hasText: /trend/i }).or(
          page.locator('[data-testid="trend-up"]').or(
            page.locator('[data-testid="trend-down"]')
          )
        );
    ...
    test('should have working quick actions', async ({ page }) => {
      ...
      if (await quickActionsSection.isVisible()) {
        ...
        const actionButtons = page.locator('text="Schedule Meeting"').or(
          page.locator('text="View Full Calendar"').or(
            page.locator('text="Calendar Settings"')
          )
        );
    ...
    test('should show weekly goal progress', async ({ page }) => {
      const progressSection = page.locator('text="Weekly Goal Progress"').or(
    ...
    test('should be responsive on mobile devices', async ({ page }) => {
      ...
      await expect(page.locator('text="Dashboard Overview"').or(
  ...
  test.describe('Design System Consistency', () => {
    ...
    test('should use consistent shadcn/ui components', async ({ page }) => {
      ...
      const cards = page.locator('[data-testid="card"]').or(
        page.locator('.border').or(
          page.locator('.rounded')
        )
      );
    ...
    test('should maintain consistent typography hierarchy', async ({ page }) => {
      ...
      if (await h1Elements.first().isVisible()) {
        const h1Styles = await h1Elements.first().evaluate(el => 
