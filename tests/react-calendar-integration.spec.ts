...
test.describe('React Calendar Integration', () => {
  ...
  test.describe('Date Selection and Event Management', () => {
    test('should select dates by clicking on calendar tiles', async ({ page }) => {
      ...
      const dateTile = page.locator('.react-calendar__tile:not(.react-calendar__tile--neighboringMonth)').first();
    ...
    test('should show event indicators on calendar dates', async ({ page }) => {
      ...
      const eventIndicators = page.locator('.event-indicator, [data-event-count], .priority-indicator');
    ...
    test('should display event priority with color coding', async ({ page }) => {
      ...
      for (let i = 0; i < priorities.length; i++) {
        const dateTile = page.locator('.react-calendar__tile:not(.react-calendar__tile--neighboringMonth)').nth(i);
        await dateTile.click();
        await page.click('button:has-text("Add Event")');
  ...
  test.describe('Event Sidebar and Details', () => {
    ...
    test('should show event statistics and analytics', async ({ page }) => {
      ...
      for (let i = 1; i <= 4; i++) {
        const dateTile = page.locator('.react-calendar__tile:not(.react-calendar__tile--neighboringMonth)').nth(i-1);
        await dateTile.click();
        await page.click('button:has-text("Add Event")');
        ...
        if (await modal.isVisible()) {
          ...
          await page.selectOption('[data-testid="event-priority-select"]', i <= 2 ? 'high' : 'medium');
          await page.click('[data-testid="save-event-btn"]');
          await page.waitForTimeout(300);
        }
      }
    ...
    test('should allow deleting events from sidebar', async ({ page }) => {
      ...
      await dateTile.click();
      const deleteBtn = page.locator('[data-testid="delete-event-btn"], button[title*="Delete"], .delete-button');
  ...
  test.describe('Calendar Tile Customization', () => {
    ...
    test('should handle weekend highlighting', async ({ page }) => {
      ...
      for (const tile of tiles) {
        ...
        if (className?.includes('weekend') || className?.includes('saturday') || className?.includes('sunday')) {
  ...
  test.describe('Performance and Loading', () => {
    ...
    test('should handle rapid date selection efficiently', async ({ page }) => {
      const tiles = page.locator('.react-calendar__tile:not(.react-calendar__tile--neighboringMonth)');
