...
test.describe('Responsive Layout Tests', () => {
  ...
  for (const viewport of viewports) {
    test(`proper layout on ${viewport.name} (${viewport.width}x${viewport.height})`, async ({
      page,
    }) => {
...
test.describe('Performance with Improved Layout', () => {
  ...
  test('memory usage remains reasonable with expanded layout', async ({ page }) => {
    ...
    if (metrics) {
      ...
      console.log(
        `Memory usage: ${metrics.usedJSHeapSize.toFixed(2)}MB / ${metrics.totalJSHeapSize.toFixed(2)}MB`
      );
    }
  });
});
