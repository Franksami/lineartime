...
function run(cmd) {
  try {
    ...
    const exitCodeInfo =
      error.status !== undefined ? `Exit code: ${error.status}` : 'Exit code: unknown';
...
const banned = [
  'components/calendar/CommandCenterCalendarVertical', // Foundation protection - vertical layout banned
...
const tokenRecommendations = [
  {
    pattern: /text-(red|green|blue|yellow|purple|orange|pink)-[0-9]/,
    replacement: 'text-destructive, text-primary, text-muted-foreground',
  },
  {
    pattern: /bg-(red|green|blue|yellow|purple|orange|pink)-[0-9]/,
    replacement: 'bg-destructive, bg-primary, bg-muted',
  },
  {
    pattern: /border-(red|green|blue|yellow|purple|orange|pink)-[0-9]/,
    replacement: 'border-destructive, border-primary, border-border',
  },
];
...
for (const bannedPath of banned) {
  try {
    ...
    const rg = run(
      `rg -n -F "${bannedPath.replace(/"/g, '\\"')}" --hidden --glob '!node_modules/**'`
    );
    if (!rg.trim()) continue;
...
if (violations.length) {
  ...
  const criticalViolations = violations.filter(
    (v) => v.includes('CommandCenterCalendarVertical') || v.includes('MobileCalendarView')
  );
