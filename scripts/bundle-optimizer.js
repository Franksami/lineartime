...
const DEPENDENCY_ANALYSIS = {
  ...
  heavy_deps: [
    {
      ...
      size: '2.1MB',
      reason: 'Icon library bloat',
      replacement: 'lucide-react (already used)',
    },
    {
    ...
    },
    {
      ...
      size: '850KB',
      reason: 'Unused UI library',
      replacement: 'Remove completely',
    },
    {
      name: '@emotion/react',
      size: '420KB',
      reason: 'Chakra dependency',
      replacement: 'Remove with Chakra',
    },
    {
      name: '@emotion/styled',
      size: '380KB',
    ...
    },
    {
      name: '@mantine/core',
      size: '1.2MB',
      reason: 'Alternative UI library',
      action: 'Consider removal',
    },
    {
      name: '@mantine/dates',
      size: '280KB',
      reason: 'Date picker alternative',
      replacement: 'Use react-datepicker',
    },
    {
      name: '@mantine/form',
      size: '180KB',
      reason: 'Form library',
      replacement: 'react-hook-form (already used)',
    },
    {
      name: '@mantine/hooks',
      size: '150KB',
      reason: 'Utility hooks',
      replacement: 'Custom hooks or remove',
    },
    {
      name: '@mantine/notifications',
      size: '120KB',
      reason: 'Toast system',
      replacement: 'sonner (already used)',
    },
    {
      name: '@mantine/spotlight',
      size: '200KB',
      reason: 'Command palette',
      replacement: 'cmdk (already used)',
    },
    {
      name: 'antd',
      size: '2.8MB',
      reason: 'Heavy UI library',
      action: 'Remove or replace with shadcn',
    },
  ],
  ...
  medium_deps: [
    ...
    { name: 'primeflex', size: '45KB', reason: 'CSS utility', replacement: 'Tailwind CSS' },
    {
      name: 'moment',
      size: '290KB',
      reason: 'Heavy date library',
      replacement: 'date-fns (already used)',
    },
  ],
  ...
  light_deps: [
    ...
    },
    {
      ...
      action: 'Bundle with async import',
    },
    {
      name: 'recharts',
      size: '380KB',
      reason: 'Charts library',
    ...
    try {
      ...
      srcFiles.forEach((file) => {
        ...
        const namespaceImports =
          content.match(/import\s+\*\s+as\s+\w+\s+from\s+['"][^'"]+['"]/g) || [];
...
`;
  ...
  calculateSavings(deps) {
    return deps
      .reduce((total, dep) => {
        const analysis = [
          ...DEPENDENCY_ANALYSIS.heavy_deps,
          ...DEPENDENCY_ANALYSIS.medium_deps,
        ].find((d) => d.name === dep);
  ...
  run() {
    console.log('🚀 Command Center Calendar Bundle Size Optimization Analysis\n');
    ...
    console.log(
      `   - scripts/migration/ (${Object.keys(migrationScripts).length} migration scripts)`
    );
