...
const calendarLibraries = {
  ...
  'primereact': { size: '280KB', category: 'primereact', note: 'Full suite - calendar is ~30KB portion' },
...
const usageAnalysis = {
  ...
  fullcalendar: { priority: 'HIGH', files: ['LinearCalendarPro.tsx', 'FullCalendarView.tsx'], removable: false },
  ...
  'react-infinite-calendar': { priority: 'LOW', files: ['ReactInfiniteCalendarView.tsx'], removable: true },
...
function analyzeCalendarBundles() {
  ...
  Object.entries(categories).forEach(([category, libs]) => {
    ...
    const usage = usageAnalysis[category] || usageAnalysis[libs[0].name] || { priority: 'UNKNOWN', removable: 'UNKNOWN' };
  ...
  console.log('\n' + '=' .repeat(70));
  console.log(`\n💾 Bundle Size Summary:`);
  console.log(`   Total Calendar Libraries: ${totalEstimatedSize}KB (~${Math.round(totalEstimatedSize/1024 * 10)/10}MB)`);
  console.log(`   Potential Savings: ${potentialSavings}KB (~${Math.round(potentialSavings/1024 * 10)/10}MB)`);
  console.log(`   Optimization Target: 800KB (Phase 2 Goal)`);
  console.log(`   Achievable Savings: ${potentialSavings >= 800 ? '✅ Goal Achievable' : '⚠️  May need additional optimization'}`);
