...
class ComponentTokenValidator {
  ...
  async validateComponentMigrations() {
    ...
    for (const dir of componentDirs) {
      ...
      for (const file of files) {
        ...
        const hasMigrationImports =
          content.includes('useComponentTokens') || content.includes('ComponentTokenRegistry');
    ...
    if (this.migrationStats.coverage < 30) {
      ...
      this.warnings.push(
        `Medium migration coverage: ${this.migrationStats.coverage}% (target: >70%)`
      );
    } else {
  ...
  async validateTokenReferences() {
    ...
    const baseTokenPatterns = ['colors', 'spacing', 'shadows', 'radii', 'typography', 'motion'];
    ...
    baseTokenPatterns.forEach((pattern) => {
      ...
      if (referencesUsingPattern.length > 0) {
        this.passed.push(
          `Base token references validated: ${pattern} (${referencesUsingPattern.length} refs)`
        );
      } else {
    ...
    const selfReferences = tokenReferences.filter((ref) => {
      ...
      return registryContent.includes(`"${cleanRef}":`) && registryContent.includes(`"${ref}"`);
    });

    if (selfReferences.length > 0) {
      this.errors.push(
        `Potential circular token references detected: ${selfReferences.join(', ')}`
      );
    } else {
  ...
  async assessPerformanceImpact() {
    ...
    const migrationUtilityPath = path.join(
      this.projectRoot,
    ...
    const registrySize = fs.statSync(
      path.join(this.projectRoot, 'lib/design-system/component-tokens/ComponentTokenRegistry.ts')
    ).size;
    ...
    if (actualSizeKB > sizeLimitKB) {
      this.warnings.push(
        `Component token system size: ${actualSizeKB}KB (target: <${sizeLimitKB}KB)`
      );
    } else {
    ...
    const memoryLeakPatterns = [/new Map\(\)/g, /new Set\(\)/g, /addEventListener/g];
    ...
    if (potentialLeaks > 5) {
      ...
      this.passed.push(
        `Memory allocation patterns within acceptable range: ${potentialLeaks} instances`
      );
    }
  }
  ...
  async calculateGovernanceScore() {
    ...
    const weights = {
      ...
      migration: 0.3, // Migration progress coverage
      references: 0.2, // Token reference validity
      performance: 0.15, // Performance impact
      compliance: 0.1, // Standards compliance
    };
    ...
    const referencesScore = Math.min(
      100,
      this.passed.filter((p) => p.includes('reference')).length * 20
    );
    const performanceScore = Math.max(
      0,
      100 - this.warnings.filter((w) => w.includes('performance')).length * 15
    );
