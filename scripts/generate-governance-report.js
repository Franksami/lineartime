...
class GovernanceReportGenerator {
  ...
  async collectTokenGovernance() {
    ...
    try {
      ...
      const tokenResult = execSync('node scripts/validate-tokens.js 2>&1 || true', { encoding: 'utf8' });
  ...
  async collectAccessibilityGovernance() {
    ...
    try {
      const a11yResult = execSync('node scripts/accessibility-check.js 2>&1 || true', { encoding: 'utf8' });
  ...
  async collectPerformanceGovernance() {
    ...
    try {
      const perfResult = execSync('node scripts/performance-check.js 2>&1 || true', { encoding: 'utf8' });
  ...
  async collectMotionGovernance() {
    ...
    try {
      const motionResult = execSync('node scripts/motion-check.js 2>&1 || true', { encoding: 'utf8' });
  ...
  checkTokenCoverage() {
    try {
      ...
      const tokenFiles = execSync('find design-tokens -name "*.json" | wc -l', { encoding: 'utf8' });
      const componentFiles = execSync('find components -name "*.tsx" | wc -l', { encoding: 'utf8' });
      
      return {
        tokenFiles: parseInt(tokenFiles.trim()),
        componentFiles: parseInt(componentFiles.trim()),
        ratio: Math.min(100, Math.round((parseInt(tokenFiles.trim()) / parseInt(componentFiles.trim())) * 100))
      };
    } catch (error) {
