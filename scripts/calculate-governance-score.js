...
class GovernanceScoreCalculator {
  constructor() {
    this.weights = {
      tokens: 0.2, // 20% - Design token compliance
      accessibility: 0.25, // 25% - Accessibility compliance (highest weight)
      performance: 0.2, // 20% - Performance benchmarks
      motion: 0.15, // 15% - Motion system compliance
      i18n: 0.1, // 10% - Internationalization
      security: 0.1, // 10% - Security compliance
    };
  ...
  async getAccessibilityScore() {
    try {
      const { execSync } = require('child_process');
      const result = execSync('node scripts/accessibility-check.js 2>&1 || true', {
        encoding: 'utf8',
      });
  ...
  async getPerformanceScore() {
    try {
      const { execSync } = require('child_process');
      const result = execSync('node scripts/performance-check.js 2>&1 || true', {
        encoding: 'utf8',
      });
  ...
  displayResults(result) {
    ...
    Object.entries(result.categoryScores).forEach(([category, score]) => {
      ...
      console.log(
        `   ${category.padEnd(12)}: ${score}% (weight: ${weight}%, contribution: ${weightedContribution})`
      );
    });
