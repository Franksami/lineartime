...
const GOVERNANCE_RULES = {
  ...
  LEGACY_CONTEXT_USAGE: {
    ...
    pattern: /CalendarContext|useLinearCalendar(?!.*year-lens)/,
    allowedPaths: [
      'views/year-lens/**/*',
      'hooks/legacy/**/*',
      'contexts/legacy/**/*'
  ...
  HARDCODED_COLORS: {
    ...
    pattern: /(bg-|text-|border-|ring-)(red|blue|green|yellow|purple|pink|indigo|gray)-\d+(?!.*\/\*\s*ok\s*\*\/)/,
    allowedPaths: [],
    severity: 'warn',
...
async function scanForViolations() {
  console.log(`${colors.blue}${colors.bold}🔍 Command Workspace Governance Validation${colors.reset}\\n`)
  ...
  if (violations.length === 0) {
    ...
    console.log(`${colors.green}🏗️ Command Workspace architecture compliance verified${colors.reset}\\n`)
    return true
  }
  
  console.log(`${colors.red}${colors.bold}❌ Found ${violations.length} governance violations:${colors.reset}\\n`)
  ...
  for (const [ruleName, ruleViolations] of Object.entries(violationsByRule)) {
    ...
    console.log(`${severityColor}${colors.bold}${rule.severity.toUpperCase()}: ${rule.description}${colors.reset}`)
  ...
  // Summary
  if (errors.length > 0) {
    console.log(`${colors.red}${colors.bold}🚨 ${errors.length} ERRORS must be fixed before deployment${colors.reset}`)
  }
  if (warnings.length > 0) {
    console.log(`${colors.yellow}⚠️  ${warnings.length} warnings should be addressed${colors.reset}`)
  }
