...
export type EventsAction =
  ...
  | { type: 'APPLY_FILTERS' }
  | { type: 'UPDATE_SYNC_STATUS'; payload: { provider: keyof EventsState['syncStatus']; status: Partial<SyncStatus> } }
...
function applyFilters(events: Event[], filters: EventFilter, searchQuery: string): Event[] {
  ...
  if (filters.providers.length > 0) {
    filtered = filtered.filter(event =>
      filters.providers.includes(event.provider || 'local')
    )
  }
...
export interface EventsContextValue {
  ...
  // Sync operations
  updateSyncStatus: (provider: keyof EventsState['syncStatus'], status: Partial<SyncStatus>) => void
...
export function EventsProvider({ children }: EventsProviderProps) {
  ...
  // Sync operations
  const updateSyncStatus = useCallback((provider: keyof EventsState['syncStatus'], status: Partial<SyncStatus>) => {
  ...
  const startBulkOperation = useCallback((type: EventsState['bulkOperation']['type'], total: number) => {
...
export function useEventsCRUD() {
  ...
  return {
    events: state.events,
    selectedEvent: state.selectedEventId ? state.events.find(e => e.id === state.selectedEventId) : null,
