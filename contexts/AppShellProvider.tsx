...
const DEFAULT_STATE: AppShellState = {
  ...
  openTabs: [
    { id: 'week-tab', view: 'week', title: 'Week View', pinned: true }
  ],
    ...
    immer((set, get) => ({
      ...
      loadLayout: (name: string) =>
        set((state) => {
          ...
          state.activeTabId = layout.openTabs.find(t => t.view === layout.activeView)?.id || layout.openTabs[0]?.id || ''
        }),
      ...
      recordRenderTime: (time: number) =>
        set((state) => {
          ...
          state.performance.averageRenderTime = 
            state.performance.renderTimes.reduce((a, b) => a + b, 0) / state.performance.renderTimes.length
        }),
