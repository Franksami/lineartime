...
export type NotificationsAction =
  ...
  | {
      ...
      payload: Omit<AccessibilityAnnouncement, 'id' | 'timestamp' | 'announced'>;
    }
  ...
  switch (action.type) {
    ...
    case 'ADD_ALERT': {
      ...
      const criticalAlerts =
        alert.level === 'critical' ? [alert, ...state.criticalAlerts] : state.criticalAlerts;
...
export function NotificationsProvider({ children }: NotificationsProviderProps) {
  ...
  const notify = {
    success: useCallback(
      (title: string, description?: string, options?: Partial<NotificationItem>) =>
        createNotification('success', title, description, options),
      [createNotification]
    ),
    error: useCallback(
      (title: string, description?: string, options?: Partial<NotificationItem>) =>
        createNotification('error', title, description, options),
      [createNotification]
    ),
    warning: useCallback(
      (title: string, description?: string, options?: Partial<NotificationItem>) =>
        createNotification('warning', title, description, options),
      [createNotification]
    ),
    info: useCallback(
      (title: string, description?: string, options?: Partial<NotificationItem>) =>
        createNotification('info', title, description, options),
      [createNotification]
    ),
  };
  ...
  const announce = useCallback(
    (message: string, priority: AccessibilityAnnouncement['priority'] = 'polite') => {
