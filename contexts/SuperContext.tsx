...
import { type AIContextValue, AIProvider, useAIContext } from './AIContext';
import { type CalendarContextType, CalendarProvider, useCalendarContext } from './CalendarContext';
import { type EventsContextValue, EventsProvider, useEventsContext } from './EventsContext';
import {
...
import { type UIContextValue, UIProvider, useUIContext } from './UIContext';
...
function SuperContextInner({
  children,
  enablePerformanceMonitoring = false,
  memoryCleanupInterval = 30000,
}: SuperContextProviderProps) {
  ...
  const coordination = useMemo(
    () => ({
      ...
      performBulkOperation: async (type: 'export' | 'delete' | 'sync', eventIds: string[]) => {
        ...
        try {
          ...
          notifications.notify.success(
            `Bulk ${type} completed`,
            `Processed ${eventIds.length} events`
          );
        } catch (error) {
      ...
      cleanup: () => {
        ...
        const oldSuggestions = ai.state.suggestions.filter(
          (s) =>
            Date.now() - new Date(s.metadata?.timestamp || Date.now()).getTime() >
            24 * 60 * 60 * 1000
        );
  ...
  return <SuperContext.Provider value={contextValue}>{children}</SuperContext.Provider>;
}
    ...
    <UIProvider>
      <CalendarProvider>
        <EventsProvider>
          <AIProvider>
            <NotificationsProvider>
              <SuperContextInner {...props}>{children}</SuperContextInner>
            </NotificationsProvider>
          </AIProvider>
        </EventsProvider>
      </CalendarProvider>
