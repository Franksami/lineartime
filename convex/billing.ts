...
export const getSubscriptionStatus = query({
  ...
  handler: async (ctx): Promise<{
    ...
    const now = Date.now();
    const daysUntilExpiry = subscription.currentPeriodEnd > now 
  ...
  handler: async (ctx, args): Promise<Id<"subscriptions">> => {
    ...
    if (existingSubscription) {
      ...
      await ctx.db.patch(existingSubscription._id, {
        status: args.status as "active" | "canceled" | "incomplete" | "incomplete_expired" | "past_due" | "paused" | "trialing" | "unpaid",
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    switch (args.feature) {
      ...
      case "providers":
        ...
        break;
      case "ai_scheduling":
        newUsage.aiRequestsThisMonth = Math.max(0, currentUsage.aiRequestsThisMonth + args.increment);
        break;
      case "storage":
...
export const initializeUserSubscription = internalMutation({
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    if (!user) {
      console.warn(`User with Clerk ID ${args.clerkUserId} not found for subscription initialization`);
      return;
    }
