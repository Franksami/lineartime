...
export const getPerformanceDashboard = query({
  handler: async (ctx) => {
    ...
    const hotUsers = Array.from(eventsByUser.entries()).filter(([_, count]) => count > 50).length;
...
export const trackQueryPerformance = query({
  args: {
    userId: v.id('users'),
    queryType: v.union(v.literal('simple'), v.literal('complex'), v.literal('aggregate')),
  },
  handler: async (ctx, args) => {
    ...
    if (args.queryType === 'complex' || args.queryType === 'aggregate') {
      ...
      const filtered = events.filter(
        (e) => e.title.length > 0 && (!e.categoryId || e.categoryId !== null)
      );
    ...
    return {
      ...
      summary: {
        ...
        maxDuration,
        performance:
          maxDuration < 10 ? 'excellent' : maxDuration < 50 ? 'good' : 'needs optimization',
      },
      timestamp: Date.now(),
    };
  },
...
function generateIndexRecommendations(
  ...
  if (underutilized.length > 0) {
    recommendations.push(`Consider removing underutilized indexes: ${underutilized.join(', ')}`);
  }
